...
describe("reefer_fuel_diversion — TMS (McLeod) reefer-load gate", () => {
  ...
  it("rule body: fires on the fuel-only heuristic when there is NO TMS feed (undefined)", () => {
    expect(ruleReeferFuelDiversion(base()).fired).toBe(true);
  });

  it("rule body: gates OFF when TMS shows no temperature-controlled load ran in the window (false)", () => {
    expect(ruleReeferFuelDiversion({ ...base(), reeferLoadInWindow: false }).fired).toBe(false);
  });

  it("rule body: fires and cites the load when TMS confirms a reefer load ran (true)", () => {
    const fired = ruleReeferFuelDiversion({ ...base(), reeferLoadInWindow: true });
    expect(fired.fired).toBe(true);
    expect(fired.evidence).toMatchObject({ reeferLoadInWindow: true });
  });

  it("TEMPORARILY SUPPRESSED at the engine level (catalog) — never reaches the alert pipeline until McLeod", () => {
    expect(ids(base())).not.toContain("reefer_fuel_diversion");
    expect(ids({ ...base(), reeferLoadInWindow: true })).not.toContain("reefer_fuel_diversion");
  });
});
...
describe("reefer fuel diversion (ULSD pumped into the reefer)", () => {
  ...
  it("rule body: fires when a reefer-hauling, active truck buys ULSD but no reefer fuel (fleet uses ULSR)", () => {
    expect(ruleReeferFuelDiversion(div()).fired).toBe(true);
  });

  it("rule body: does NOT fire when the fleet never codes reefer (ULSR) fuel — no basis to accuse", () => {
    expect(ruleReeferFuelDiversion(div({ orgUsesReeferFuel: false })).fired).toBe(false);
  });

  it("rule body: does NOT fire when the truck does not haul a reefer", () => {
    expect(ruleReeferFuelDiversion(div({ reeferPaired: false })).fired).toBe(false);
  });

  it("rule body: does NOT fire when the truck IS buying reefer fuel", () => {
    expect(ruleReeferFuelDiversion(div({ reeferDiversionReeferGal: 45 })).fired).toBe(false);
  });

  it("rule body: does NOT fire when the truck is inactive (little ULSD → parked reefer needs no fuel)", () => {
    expect(ruleReeferFuelDiversion(div({ reeferDiversionTractorGal: 40 })).fired).toBe(false);
  });

  it("rule body: does NOT fire on a reefer (ULSR) fill itself", () => {
    expect(ruleReeferFuelDiversion(div({ txn: txn({ gallons: 40, tankType: "reefer" }) })).fired).toBe(false);
  });

  it("engine: temporarily suppressed — a would-fire context produces NO anomaly (until McLeod)", () => {
    expect(ids(div())).not.toContain("reefer_fuel_diversion");
  });
...
describe("WP-CAP — exceeds_tank_capacity on resolved capacity + confidence tiers", () => {
  it("sensor-VERIFIED capacity fires at the tight org tolerance (5%)", () => {
    ...
    expect(ids(ctx({ vehicle: verified, txn: txn({ gallons: 130 }) }))).toContain("exceeds_tank_capacity"); // >128.1
    expect(ids(ctx({ vehicle: verified, txn: txn({ gallons: 127 }) }))).not.toContain("exceeds_tank_capacity");
  });
  ...
  it("sensor capacity REVIVES detection when no capacity was ever entered (was silently dead)", () => {
    ...
    expect(ids(ctx({ vehicle: noEntry, txn: txn({ gallons: 260 }) }))).toContain("exceeds_tank_capacity");
    expect(ids(ctx({ vehicle: { ...vehicle, tankCapacityGal: 0 }, txn: txn({ gallons: 260 }) }))).not.toContain("exceeds_tank_capacity");
  });
  ...
  it("self-corroboration does NOT rescue a fill whose rise is too small for the bill (fuel didn't go in)", () => {
    ...
    expect(ids(ctx({ txn: txn({ gallons: 165 }), tankPctBefore: 40, tankPctAfter: 62 }))).toContain("exceeds_tank_capacity");
  });
  ...
  it("self-corroboration is NEVER applied to sensor-verified capacity (one fill can't outvote physics history)", () => {
    ...
    expect(ids(ctx({ vehicle: verified, txn: txn({ gallons: 165 }), tankPctBefore: 42, tankPctAfter: 92 }))).toContain("exceeds_tank_capacity");
  });

  it("rise-short corroboration is carried in evidence for the reviewer", () => {
    const out = runAllRules(ctx({ txn: txn({ gallons: 165 }), tankObservedRiseGal: 40 })).find((r) => r.ruleId === "exceeds_tank_capacity");
    expect(out).toBeTruthy();
...
describe("WP-ATTR — logbook-contradicted attribution gates the vehicle-physics rules", () => {
  it("an attribution-suspect fill fires NO vehicle-relative rules (data-quality, not fraud)", () => {
    ...
    const c = ctx({ txn: txn({ gallons: 150 }), windowGallons: 400, windowMiles: 600, attributionSuspect: true });
    const out = ids(c);
    expect(out).not.toContain("exceeds_tank_capacity");
...
describe("WP-BEH — rapid_repeat same-site by real distance", () => {
  const at = (lat: number, lng: number, over: Partial<TxnView> = {}) => txn({ stationLat: lat, stationLng: lng, ...over });
  const prev = (lat: number, lng: number) => ({ ...txn(), id: "prev", fueledAt: "2026-06-10T16:30:00Z", odometer: 99990, stationLat: lat, stationLng: lng });

  it("two pins across the road (<1 mi) are ONE site → exempt, even with different names", () => {
    const c = ctx({ txn: at(41.13, -104.8, { locationText: "FJ-CHEYENNE 759" }), previousTxn: { ...prev(41.135, -104.8), locationText: "Pilot Cheyenne" } });
  ...
  it("two stations in the SAME city but miles apart now fire (string fallback wrongly exempted them)", () => {
    const c = ctx({ txn: at(41.13, -104.8, { city: "Cheyenne", state: "WY", locationText: null }), previousTxn: { ...prev(41.2, -104.6), city: "Cheyenne", state: "WY", locationText: null } });
    expect(ids(c)).toContain("rapid_repeat_fueling");
  });
  it("no coordinates → string fallback behaves exactly as before", () => {
    const c = ctx({ txn: txn({ locationText: "SITE A" }), previousTxn: { ...txn(), id: "prev", fueledAt: "2026-06-10T16:30:00Z", odometer: 99990, locationText: "SITE A" } });
    expect(ids(c)).not.toContain("rapid_repeat_fueling");
  });
});

describe("WP-BEH — impossible_travel (card in two places one truck can't be)", () => {
  const fillAt = (isoTime: string, lat: number, lng: number, over: Partial<TxnView> = {}): TxnView =>
    ({ ...txn(), fueledAt: isoTime, stationLat: lat, stationLng: lng, ...over });
  ...
  it("fires when the implied speed between distant pins is impossible, and supersedes rapid_repeat", () => {
    ...
    const c = ctx({
      ...
      previousTxn: { ...fillAt("2026-06-10T16:30:00Z", 39.74, -104.99), id: "prev", odometer: 99990 },
    });
    const out = ids(c);
  ...
  it("a feasible pair stays silent; pins under 60 mi apart never fire (pin-noise guard)", () => {
    const feasible = ctx({
      txn: fillAt("2026-06-10T17:00:00Z", 41.14, -104.82),
      previousTxn: { ...fillAt("2026-06-10T14:00:00Z", 39.74, -104.99), id: "prev", odometer: 99850 },
    });
    expect(ids(feasible)).not.toContain("impossible_travel");
    const near = ctx({
      txn: fillAt("2026-06-10T17:00:00Z", 41.14, -104.82),
      previousTxn: { ...fillAt("2026-06-10T16:55:00Z", 41.5, -104.82), id: "prev", odometer: 99990 }, // ~25 mi in 5 min but < 60 mi
    });
...
describe("WP-BEH — distance-tier location_mismatch", () => {
  it("same-state but far from the station now fires, tiered by distance", () => {
    const near = runAllRules(ctx({ truckToStationMiles: 40 })).find((r) => r.ruleId === "location_mismatch");
    expect(near).toBeTruthy();
    expect(near!.severity).toBe("medium");
    const far = runAllRules(ctx({ truckToStationMiles: 150 })).find((r) => r.ruleId === "location_mismatch");
  ...
  it("inside the threshold, on a suspect station pin, or without a reliable instant → silent", () => {
    ...
    expect(ids(ctx({ truckToStationMiles: 40, locationEvidence: { dataQuality: "station_coordinate_suspect" } }))).not.toContain("location_mismatch");
    expect(ids(ctx({ truckToStationMiles: 40, txn: txn({ timeConfirmed: false }) }))).not.toContain("location_mismatch");
  });
...
describe("WP-BEH — precision-scaled tank_fill_short + chronic shortfall", () => {
  it("a clean sensor (tight sigma) tightens the tolerance: a 25-gal short on 100 gal now fires", () => {
    ...
    const c = ctx({ vehicle: clean, txn: txn({ gallons: 100 }), tankFillShortGal: 25, tankObservedRiseGal: 75 });
    ...
    const legacy = ctx({ vehicle: reliable, txn: txn({ gallons: 100 }), tankFillShortGal: 25, tankObservedRiseGal: 75 });
  ...
  it("a noisy sensor keeps the wide band (sigma never widens beyond 30%, never tightens below 8%)", () => {
    ...
    const c = ctx({ vehicle: noisy, txn: txn({ gallons: 100 }), tankFillShortGal: 25, tankObservedRiseGal: 75 });
  ...
  it("chronic shortfall fires on a persistent one-direction residual across the window", () => {
    ...
    const c = ctx({ vehicle: clean, tankResidualWindow: { fills: 8, sumShortGal: 70, totalBilledGal: 800 } });
  ...
  it("symmetric noise (residual ≈ 0), few fills, or an unreliable sensor stay silent", () => {
    const clean: VehicleView = { ...reliable, tankRatioSigma: 0.03 };
    expect(ids(ctx({ vehicle: clean, tankResidualWindow: { fills: 8, sumShortGal: 5, totalBilledGal: 800 } }))).not.toContain("tank_chronic_short");
    expect(ids(ctx({ vehicle: clean, tankResidualWindow: { fills: 4, sumShortGal: 70, totalBilledGal: 400 } }))).not.toContain("tank_chronic_short");
    expect(ids(ctx({ vehicle: { ...vehicle, tankRatioSigma: 0.03 }, tankResidualWindow: { fills: 8, sumShortGal: 70, totalBilledGal: 800 } }))).not.toContain("tank_chronic_short"); // not learned-reliable
  });
});
...
describe("WP-BEH — driver-not-working from the ELD logbook", () => {
  it("fires from the ELD source alone, from TMS alone, and reports the source", () => {
    const eld = runAllRules(ctx({ driverOffDutyAtFill: true })).find((r) => r.ruleId === "fuel_while_driver_home");
    expect(eld).toBeTruthy();
    expect(eld!.evidence.source).toBe("eld");
    const tms = runAllRules(ctx({ driverHomeAtFill: true })).find((r) => r.ruleId === "fuel_while_driver_home");
