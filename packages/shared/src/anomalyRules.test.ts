...
describe("tank_space_exceeded (physical: can't add more than the tank holds)", () => {
  ...
  it("suppresses when the tank was already nearly full — contradictory/mistimed reading", () => {
    ...
    expect(
      ids(ctx({ vehicle: reliable, tankPctBefore: 99, txn: txn({ gallons: 150 }) })),
    ).not.toContain("tank_space_exceeded");
  });
  ...
  it("is suppressed when the post-fill sensor shows the fuel actually FIT (stale pre-fill sample — audit A2.3)", () => {
    ...
    expect(
      ids(ctx({ vehicle: reliable, tankPctBefore: 60, tankObservedRiseGal: 90 })),
    ).not.toContain("tank_space_exceeded");
  });
  it("still fires when the observed rise is SHORT (fuel didn't all go in → corroborates the overflow)", () => {
    ...
    expect(ids(ctx({ vehicle: reliable, tankPctBefore: 60, tankObservedRiseGal: 40 }))).toContain(
      "tank_space_exceeded",
    );
  });
  it("still fires on the pre-fill sample alone when no post-fill rise was measured (behaviour unchanged)", () => {
    expect(ids(ctx({ vehicle: reliable, tankPctBefore: 60, tankObservedRiseGal: null }))).toContain(
      "tank_space_exceeded",
    );
  });
});
...
describe("Tier 2 — capacity (fuel vehicles only)", () => {
  ...
  it("implausible_topoff fires when gallons far exceed consumption (reliable-sensor truck)", () => {
    ...
    expect(ids(ctx({ vehicle: reliable, txn: txn({ gallons: 110, odometer: 99407 }) }))).toContain(
      "implausible_topoff",
    );
  });
  it("capacity rule is skipped for electric vehicles (gating H1)", () => {
    ...
    expect(ids(ctx({ vehicle: ev, txn: txn({ gallons: 150 }) }))).not.toContain(
...
describe("Phase 2 — learned combined tank capacity", () => {
  ...
  it("effectiveCapacityGal only RAISES capacity above an under-entered nameplate, never lowers it", () => {
    ...
    expect(
      effectiveCapacityGal({ ...vehicle, tankCapacityGal: 120, observedMaxFillGal: 210 }),
    ).toBe(210); // dual-tank raise
    expect(
      effectiveCapacityGal({ ...vehicle, tankCapacityGal: 240, observedMaxFillGal: 210 }),
    ).toBe(240); // never lowers
  });
  ...
  it("a legitimate both-tank fill fires exceeds_tank_capacity ONLY before capacity is learned", () => {
    ...
    expect(ids(ctx({ vehicle: singleTankEntered, txn: bothTankFill }))).toContain(
      "exceeds_tank_capacity",
    );
    ...
    expect(ids(ctx({ vehicle: learned, txn: bothTankFill }))).not.toContain(
      "exceeds_tank_capacity",
    );
  });
});
...
describe("Tier 3 — efficiency", () => {
  it("mpg_deviation fires when MPG is well below baseline (reliable-sensor truck)", () => {
    ...
    expect(ids(ctx({ vehicle: reliable, txn: txn({ odometer: 99450 }) }))).toContain(
      "mpg_deviation",
    );
  });
  ...
  it("mpg_deviation is suppressed on a TOO-SMALL fill even on a reliable truck (audit A2.4)", () => {
    ...
    expect(ids(ctx({ vehicle: reliable, txn: txn({ odometer: 99480, gallons: 20 }) }))).toContain(
      "mpg_deviation",
    );
    expect(
      ids(ctx({ vehicle: reliable, txn: txn({ odometer: 99440, gallons: 10 }) })),
    ).not.toContain("mpg_deviation");
  });
  it("cold-weather derate: a borderline drop fires in summer but not in deep winter", () => {
    ...
    expect(
      ids(
        ctx({ vehicle: reliable, txn: txn({ ...borderline, fueledAt: "2026-06-10T17:00:00Z" }) }),
      ),
    ).toContain("mpg_deviation");
    expect(
      ids(
        ctx({ vehicle: reliable, txn: txn({ ...borderline, fueledAt: "2026-12-10T17:00:00Z" }) }),
      ),
    ).not.toContain("mpg_deviation");
  });
  it("cold-weather derate never HIDES a severe drop even in winter", () => {
    ...
    expect(
      ids(
        ctx({ vehicle: reliable, txn: txn({ odometer: 99450, fueledAt: "2026-12-10T17:00:00Z" }) }),
      ),
    ).toContain("mpg_deviation");
  });
  ...
  it("mpg_sustained_decline fires on a downward trend", () => {
    ...
    mpgs.forEach((m, i) => {
      odo += m * 100;
      recent.push(
        txn({ id: `r${i}`, odometer: odo, gallons: 100, fueledAt: `2026-05-${10 + i}T17:00:00Z` }),
      );
    });
    ...
    expect(
      ids(
        ctx({
          vehicle: reliable,
          txn: cur,
          previousTxn: recent[recent.length - 1]!,
          recentTxns: recent,
        }),
      ),
    ).toContain("mpg_sustained_decline");
  });
});
...
describe("Audit fixes — P-1 (odometer typo can't poison 3 axes) & P-3 (no double-count)", () => {
  it("P-1: an odometer mismatch suppresses the miles-derived per-fill rules", () => {
    ...
    const out = ids(
      ctx({ vehicle: reliable, txn: txn({ odometer: 99450 }), crossSourceOdometer: 99480 }),
    );
  ...
  it("P-1: a huge diff (entry_suspect) also suppresses the miles-derived rules and stays low-severity", () => {
    const out = runAllRules(
      ctx({ vehicle: reliable, txn: txn({ odometer: 99450 }), crossSourceOdometer: 130000 }),
    );
  ...
  it("P-3: implausible_topoff and mpg_deviation share one axis (can't be a 2-axis alert alone)", () => {
    const r = (ruleId: RuleId): RuleResult => ({
      ruleId,
      fired: true,
      severity: "high",
      message: "",
...
describe("Tier 4 — behavioral", () => {
  it("rapid_repeat_fueling fires within the window", () => {
    expect(ids(ctx({ txn: txn({ fueledAt: "2026-06-08T18:00:00Z", odometer: 99450 }) }))).toContain(
      "rapid_repeat_fueling",
    );
  });
  it("off_hours_fueling fires at 2am local", () => {
    // 2026-06-10T07:00:00Z = 02:00 Chicago
    expect(ids(ctx({ txn: txn({ fueledAt: "2026-06-10T07:00:00Z" }) }))).toContain(
      "off_hours_fueling",
    );
  });
  it("unattributed_transaction is suppressed (data-quality, not an anomaly)", () => {
    expect(
      ids(ctx({ txn: txn({ vehicleId: null, driverId: null }), previousTxn: null })),
    ).not.toContain("unattributed_transaction");
  });
...
describe("disabledRules + helpers", () => {
  ...
  it("isOffHours handles the org timezone", () => {
    expect(
      isOffHours("2026-06-10T07:00:00Z", { start: "05:00", end: "20:00", tz: "America/Chicago" }),
    ).toBe(true);
    expect(
      isOffHours("2026-06-10T17:00:00Z", { start: "05:00", end: "20:00", tz: "America/Chicago" }),
    ).toBe(false);
  });
...
describe("reefer tank split (Phase 0)", () => {
  ...
  it("still runs tractor volume rules on a normal tractor fill", () => {
    expect(ids(ctx({ txn: txn({ gallons: 150, tankType: "tractor" }) }))).toContain(
      "exceeds_tank_capacity",
    );
  });
  ...
  it("does not fire reefer_exceeds_capacity for a normal reefer fill within the tank", () => {
    expect(
      ids(ctx({ txn: txn({ gallons: 40, tankType: "reefer" }), reeferTankCapacityGal: 50 })),
    ).not.toContain("reefer_exceeds_capacity");
  });
  ...
  it("fires reefer_overfuel_rate when window reefer gallons exceed burn + a tank", () => {
    ...
    const c = ctx({
      txn: txn({ gallons: 30, tankType: "reefer" }),
      reeferTankCapacityGal: 50,
      reeferWindowGallons: 200,
    });
...
describe("reefer_fuel_diversion — TMS (McLeod) reefer-load gate", () => {
  ...
  it("still fires and cites the load when TMS confirms a reefer load ran (true)", () => {
    const fired = runAllRules({ ...base(), reeferLoadInWindow: true }).find(
      (r) => r.ruleId === "reefer_fuel_diversion",
    );
...
describe("fuel_while_driver_home — TMS (McLeod) driver-home gate (opt-in, corroboration-only)", () => {
  ...
  it("fires and cites the home-time window when the fill landed on home time (true)", () => {
    const fired = runAllRules(ctx({ driverHomeAtFill: true })).find(
  ...
  it("reinforces an independent volume signal into an alert (two axes agree)", () => {
    const c = correlateSignals([
      homeSig(),
      { ruleId: "tank_fill_short", fired: true, severity: "medium", message: "", evidence: {} },
    ]);
    expect(c.level).toBe("alert");
...
describe("hardening — time confidence (EFS auth-time vs telematics)", () => {
  it("suppresses off-hours + rapid-repeat when the posted time is UNcorroborated (timeConfirmed=false)", () => {
    ...
    const c = ctx({
      txn: txn({ fueledAt: "2026-06-10T07:00:00Z", timeConfirmed: false, odometer: 99450 }),
    });
    const out = ids(c);
    expect(out).not.toContain("off_hours_fueling");
    expect(out).not.toContain("rapid_repeat_fueling");
  ...
  it("rapid-repeat measures the interval between telematics eventAt instants", () => {
    ...
    const prev = {
      ...txn(),
      id: "prev",
      fueledAt: "2026-06-08T02:00:00Z",
      eventAt: "2026-06-08T17:00:00Z",
      odometer: 99400,
      timeConfirmed: true,
    };
    const c = ctx({
      previousTxn: prev,
      txn: txn({
        fueledAt: "2026-06-08T03:00:00Z",
        eventAt: "2026-06-08T18:00:00Z",
        odometer: 99450,
        timeConfirmed: true,
      }),
    });
...
describe("hardening — odometer correctness (±5 cross-source)", () => {
  ...
  it("does NOT fire on a GPS-sourced odometer even with a large diff (GPS bias, display-only)", () => {
    ...
    expect(
      ids(ctx({ crossSourceOdometer: 100020, crossSourceOdometerSource: "gps" })),
    ).not.toContain("odometer_mismatch");
    expect(
      ids(ctx({ crossSourceOdometer: 100020, crossSourceOdometerSource: "reconstructed" })),
    ).not.toContain("odometer_mismatch");
  });
  it("still fires on an OBD-sourced odometer beyond tolerance", () => {
    expect(ids(ctx({ crossSourceOdometer: 100020, crossSourceOdometerSource: "obd" }))).toContain(
      "odometer_mismatch",
    );
  });
  ...
  it("a learned per-vehicle offset absorbs a constant dash↔Samsara gap (no false flag)", () => {
    ...
    expect(ids(ctx({ vehicle: calibrated, crossSourceOdometer: 98800 }))).not.toContain(
      "odometer_mismatch",
    );
  });
  ...
  it("still fires when a fill deviates from the established offset beyond tolerance", () => {
    ...
    expect(ids(ctx({ vehicle: calibrated, crossSourceOdometer: 98750 }))).toContain(
      "odometer_mismatch",
    );
  });
});
...
describe("Phase 5 — implausibly huge odometer diff is DATA QUALITY, not theft", () => {
  ...
  it("is gated OBD-only like odometer_mismatch (no GPS/reconstructed)", () => {
    expect(
      ids(ctx({ crossSourceOdometer: 127001, crossSourceOdometerSource: "gps" })),
    ).not.toContain("odometer_entry_suspect");
  });
});

describe("learnTankSensorReliability", () => {
  const fills = (ratios: number[]) =>
    ratios.map((r) => ({ observedRiseGal: r * 100, billedGallons: 100 }));
  ...
  it("returns null until enough samples (evidence floor is 8 — audit A2.2)", () => {
    ...
    expect(
      learnTankSensorReliability(fills([1.0, 0.99, 1.01, 1.0, 0.98, 1.02, 1.0, 0.97]))!.reliable,
    ).toBe(true);
  });
  ...
  it("treats physically-impossible ratios (>1: rose more than bought) as NOT reconciling", () => {
    ...
    expect(
      learnTankSensorReliability(fills([1.3, 1.25, 1.35, 1.28, 1.4, 1.32, 1.27, 1.31]))!.reliable,
    ).toBe(false);
  });
  it("marks a MOSTLY-single-tank truck UNreliable when it has a tail of both-tank (short) fills", () => {
    ...
    const r = learnTankSensorReliability(
      fills([1.0, 0.98, 1.02, 1.0, 0.97, 1.03, 1.0, 0.99, 1.01, 0.7, 0.68, 0.72]),
    );
  ...
  it("stays reliable for a genuine single-tank truck with just one odd fill", () => {
    ...
    const r = learnTankSensorReliability(
      fills([1.0, 0.98, 1.02, 1.0, 0.97, 1.03, 1.0, 0.99, 1.01, 0.96, 1.04, 0.6]),
    );
    expect(r!.reliable).toBe(true);
  });
});
...
describe("Phase 3 — robust over-fuel window miles", () => {
  const row = (
    ...
    samsaraOdometer: number | null = null,
    samsaraSource: string | null = null,
  ...
  it("suppresses (null) when the CLEAN OBD odometer shows no movement, without falling back to entered", () => {
    ...
    expect(
      robustWindowMiles([row(100000, 500000, "obd"), row(100600, 500000, "obd")]).miles,
    ).toBeNull();
  });
});
...
describe("hardening — card sharing", () => {
  ...
  it("does not fire for a single-vehicle card", () => {
    expect(ids(ctx({ txn: txn({ cardRef: "93509" }), cardVehicleCountInWindow: 1 }))).not.toContain(
      "card_multi_vehicle",
    );
  });
});
  ...
  it("fires (high) only when telematics confirms a different state at the fueling time", () => {
    const fired = runAllRules(
      ctx({
        ...
        locationEvidence: { efsState: "GA", samsaraState: "TX" },
      }),
    );
...
describe("hardening — tank fill short (Samsara, advisory)", () => {
  ...
  it("fires (low) when the tank rose far less than billed gallons (monitored tank configured)", () => {
    const fired = runAllRules(
      ctx({ vehicle: monitored, tankFillShortGal: 65, tankObservedRiseGal: 25 }),
    );
  ...
  it("does NOT fire when the sensor is not learned-reliable (dual-tank/unknown) — even on a large shortfall", () => {
    ...
    expect(ids(ctx({ tankFillShortGal: 65, tankObservedRiseGal: 25 }))).not.toContain(
      "tank_fill_short",
    );
  });
  ...
  it("does NOT fire on a small sensor-noise shortfall within tolerance (monitored tank configured)", () => {
    ...
    expect(
      ids(ctx({ vehicle: monitored, tankFillShortGal: 0.4, tankObservedRiseGal: 89.6 })),
    ).not.toContain("tank_fill_short");
...
describe("reefer fuel diversion (ULSD pumped into the reefer)", () => {
  ...
  it("does NOT fire on a reefer (ULSR) fill itself", () => {
    expect(ids(div({ txn: txn({ gallons: 40, tankType: "reefer" }) }))).not.toContain(
...
describe("WP2 P-1 hardening — OBD-basis miles survive a bad odometer entry", () => {
  ...
  const obdTxn = txn({
    odometer: 250000,
    samsaraOdometer: 100000,
    samsaraOdometerSource: "obd",
    gallons: 200,
  });
  const obdPrev: TxnView = {
    ...txn(),
    id: "prev",
    fueledAt: "2026-06-08T17:00:00Z",
    odometer: 99400,
    samsaraOdometer: 99400,
    samsaraOdometerSource: "obd",
  };
...
describe("WP2 — explainCaseOutcome (every outcome is explainable, including clear)", () => {
  ...
  it("alert explains overwhelming vs corroborated", () => {
    expect(explainCaseOutcome("alert", 90, [sig("tank_space_exceeded", "volume", 90)])).toMatch(
      /overwhelming/i,
    );
    expect(
      explainCaseOutcome("alert", 125, [
        sig("cumulative_overfuel", "consumption", 75),
        sig("location_mismatch", "location", 50),
      ]),
...
describe("WP3b card_multi_vehicle (169-false-alarm regression suite)", () => {
  it("THE 169 CLASS: a LEARNED-assignment mismatch alone (count 1) NEVER fires — era changes and", () => {
    ...
    const c = ctx({
      txn: txn({ cardRef: "7083050030281917521" }),
      cardVehicleCountInWindow: 1,
      cardAssignedVehicleId: "other-truck",
    });
  ...
  it("a MANUAL (human-declared) assignment mismatch fires even as a single event", () => {
    const c = ctx({
      txn: txn({ cardRef: "7083050030281917521" }),
      cardVehicleCountInWindow: 1,
      cardManualAssignedVehicleId: "other-truck",
    });
    ...
    expect(
      ids(
        ctx({
          txn: txn({ cardRef: "7083050030281917521" }),
          cardVehicleCountInWindow: 1,
          cardManualAssignedVehicleId: "v1",
        }),
      ),
  ...
  it("the classic ≥2-vehicles split-use signal still fires, enriched by the as-of assignment", () => {
    const out = runAllRules(
      ctx({
        txn: txn({ cardRef: "93509" }),
        cardVehicleCountInWindow: 2,
        cardAssignedVehicleId: "other-truck",
      }),
    ).find((r) => r.ruleId === "card_multi_vehicle");
    expect(out).toBeTruthy();
  ...
  it("an unattributed fill never fires the manual-mismatch variant", () => {
    expect(
      ids(
        ctx({
          txn: txn({ cardRef: "93509", vehicleId: null }),
          previousTxn: null,
          cardVehicleCountInWindow: 1,
          cardManualAssignedVehicleId: "other-truck",
        }),
...
describe("WP3c card_multi_vehicle identifiability gate (masked-card false-alert class)", () => {
  it("an unidentifiable card (masked ref, no control number) never fires, whatever the count says", () => {
    const c = ctx({
      txn: txn({ cardRef: "708305XXXXXX1234" }),
      cardIdentifiable: false,
      cardVehicleCountInWindow: 4,
    });
...
describe("WP4 odometer_regression (tolerance + OBD arbitration)", () => {
  ...
  it("OBD agrees with THIS entry → the previous entry was wrong — silent (data-quality on prev)", () => {
    const out = ids(
      ctx({
        txn: txn({ odometer: 99300 }),
        crossSourceOdometer: 99302,
        crossSourceOdometerSource: "obd",
      }),
    );
...
describe("WP4 daily-cap / implausible-jump are ENTERED-basis checks (team drivers on OBD never flag)", () => {
  ...
  it("a genuine team-style OBD span (1,200 mi/day) never fires the daily cap", () => {
    const cur = txn({
      fueledAt: "2026-06-09T17:00:00Z",
      odometer: 101800,
      samsaraOdometer: 101800,
      samsaraOdometerSource: "obd",
      fueledAtPrecision: "date",
    });
    const prev: TxnView = {
      ...txn(),
      id: "prev",
...
describe("WP6 temperature-aware cold derate (real cold governs; calendar is the floor)", () => {
  ...
  it("mpg_deviation consumes the fill's ambient temperature", () => {
    ...
    const cold = runAllRules(
      ctx({ vehicle: reliable, intermediateGallons: 60, ambientTempF: 10 }),
    ).find((r) => r.ruleId === "mpg_deviation");
    expect(cold!.evidence.coldWeatherDeratePct).toBe(10);
    const warm = runAllRules(
      ctx({ vehicle: reliable, intermediateGallons: 60, ambientTempF: 70 }),
    ).find((r) => r.ruleId === "mpg_deviation");
...
describe("WP6 summarizeFillGates (honest-absence surface)", () => {
  ...
  it("reliable sensor + measurable fill + OBD odometer → nothing gated", () => {
    const gates = summarizeFillGates(
      computeFillConfidence(ctx({ vehicle: reliable, crossSourceOdometerSource: "obd" })),
    );
    expect(gates.ineligible).toEqual([]);
  });
  it("GPS-derived odometer source gates the absolute mismatch checks", () => {
    const gates = summarizeFillGates(
      computeFillConfidence(ctx({ vehicle: reliable, crossSourceOdometerSource: "gps" })),
    );
    expect(gates.ineligible).toEqual(
      expect.arrayContaining(["odometer_mismatch", "odometer_entry_suspect"]),
    );
  });
});
...
describe("WP7 rapid_repeat_fueling (same-station split purchases exempt)", () => {
  ...
  it("two swipes at the SAME station minutes apart (pre-auth split) never fire", () => {
    const prev: TxnView = {
      ...txn(),
      ...prevAt,
      locationText: "PILOT RAYVILLE 335",
      state: "LA",
      city: "RAYVILLE",
    };
    const cur = txn({
  ...
  it("same city/state fallback also exempts when site names are missing", () => {
    ...
    expect(
      ids(ctx({ txn: txn({ city: "DALLAS", state: "tx", odometer: 99400 }), previousTxn: prev })),
    ).not.toContain("rapid_repeat_fueling");
  });
  it("DIFFERENT stations inside the window still fire, with combined gallons in evidence", () => {
    ...
    const out = runAllRules(
      ctx({
        txn: txn({ locationText: "LOVES #646", city: "WILLINGTON", state: "CT", odometer: 99400 }),
        previousTxn: prev,
      }),
    ).find((r) => r.ruleId === "rapid_repeat_fueling");
    expect(out).toBeTruthy();
  ...
  it("unknown location never exempts (fires as before)", () => {
    ...
    expect(ids(ctx({ txn: txn({ odometer: 99400 }), previousTxn: prev }))).toContain(
      "rapid_repeat_fueling",
    );
  });
});
...
describe("WP7 off_hours_fueling (judged in the STATION state's timezone)", () => {
  ...
  it("a fill inside office hours but before dawn at the STATION fires (station-local)", () => {
    const out = runAllRules(
      ctx({ txn: txn({ fueledAt: "2026-06-10T11:30:00Z", state: "NV" }) }),
    ).find((r) => r.ruleId === "off_hours_fueling");
    expect(out).toBeTruthy();
  ...
  it("the same instant with no station state falls back to the org timezone (no fire)", () => {
    expect(ids(ctx({ txn: txn({ fueledAt: "2026-06-10T11:30:00Z" }) }))).not.toContain(
      "off_hours_fueling",
    );
  });
  it("the 24/7 sentinel (unset org config) disables the rule entirely", () => {
    expect(
      ids(
        ctx({
          txn: txn({ fueledAt: "2026-06-10T07:00:00Z" }),
          operatingHours: { start: "00:00", end: "00:00", tz: "America/Chicago" },
        }),
      ),
...
describe("WP7 cost_outlier (market variant on the posted-price layer)", () => {
  it("≥35% over the regional posted median fires without any org config", () => {
    const out = runAllRules(ctx({ txn: txn({ pricePerGal: 6.5 }), marketPricePerGal: 4.2 })).find(
      (r) => r.ruleId === "cost_outlier",
    );
    expect(out).toBeTruthy();
    expect(out!.evidence.marketMedianPerGal).toBe(4.2);
  });
  it("a normal spread over the median stays silent; below-market never fires", () => {
    expect(ids(ctx({ txn: txn({ pricePerGal: 4.6 }), marketPricePerGal: 4.2 }))).not.toContain(
      "cost_outlier",
    );
    expect(ids(ctx({ txn: txn({ pricePerGal: 2.0 }), marketPricePerGal: 4.2 }))).not.toContain(
      "cost_outlier",
    );
  });
...
// ── WP-CAP — capacity resolver drives the capacity rules ──────────────────────────────────────────
describe("WP-CAP — exceeds_tank_capacity on resolved capacity + confidence tiers", () => {
  it("sensor-VERIFIED capacity fires at the tight org tolerance (5%)", () => {
    const verified: VehicleView = { ...vehicle, sensorCapacityGal: 122 }; // agrees with entered 120 → high
    expect(ids(ctx({ vehicle: verified, txn: txn({ gallons: 130 }) }))).toContain(
      "exceeds_tank_capacity",
    ); // >128.1
    expect(ids(ctx({ vehicle: verified, txn: txn({ gallons: 127 }) }))).not.toContain(
      "exceeds_tank_capacity",
    );
  });

  it("UNVERIFIED (entered-only) capacity needs a 15% overage for the alert-alone critical", () => {
    expect(ids(ctx({ txn: txn({ gallons: 137 }) }))).not.toContain("exceeds_tank_capacity"); // 14% over — not critical
  ...
  it("the unverified 5–15% band fires the review-grade companion instead (weight 60 → lone review)", () => {
    ...
    expect(v.map((r) => r.ruleId)).not.toContain("exceeds_capacity_unverified");
  ...
  it("sensor capacity REVIVES detection when no capacity was ever entered (was silently dead)", () => {
    const noEntry: VehicleView = { ...vehicle, tankCapacityGal: 0, sensorCapacityGal: 200 };
    expect(ids(ctx({ vehicle: noEntry, txn: txn({ gallons: 260 }) }))).toContain(
      "exceeds_tank_capacity",
    );
    expect(
      ids(ctx({ vehicle: { ...vehicle, tankCapacityGal: 0 }, txn: txn({ gallons: 260 }) })),
    ).not.toContain("exceeds_tank_capacity");
  });

  it("self-corroboration: an unverified record + this fill's own big plausible rise → record error, not theft", () => {
    // Entered 120 (unverified). 165 gal billed, tank rose 42→92 (50 pts) → implied ~330 gal — a real
    // (dual/saddle) tank the record under-states. Suppressed; divergence surfacing fixes the record.
    const out = ids(ctx({ txn: txn({ gallons: 165 }), tankPctBefore: 42, tankPctAfter: 92 }));
    expect(out).not.toContain("exceeds_tank_capacity");
    expect(out).not.toContain("exceeds_capacity_unverified");
  });

  it("self-corroboration does NOT rescue a fill whose rise is too small for the bill (fuel didn't go in)", () => {
    // 165 gal billed but the tank only rose 22 points → implied ~750 gal (impossible) → theft stands.
    expect(ids(ctx({ txn: txn({ gallons: 165 }), tankPctBefore: 40, tankPctAfter: 62 }))).toContain(
      "exceeds_tank_capacity",
    );
  });

  it("self-corroboration is NEVER applied to sensor-verified capacity (one fill can't outvote physics history)", () => {
    const verified: VehicleView = { ...vehicle, sensorCapacityGal: 122 };
    // Same big-rise shape as the record-error case above, but capacity is verified → the alert stands.
    expect(
      ids(
        ctx({ vehicle: verified, txn: txn({ gallons: 165 }), tankPctBefore: 42, tankPctAfter: 92 }),
      ),
    ).toContain("exceeds_tank_capacity");
  });

  it("rise-short corroboration is carried in evidence for the reviewer", () => {
    const out = runAllRules(ctx({ txn: txn({ gallons: 165 }), tankObservedRiseGal: 40 })).find(
      (r) => r.ruleId === "exceeds_tank_capacity",
    );
    expect(out).toBeTruthy();
    expect(out!.evidence.riseCorroborates).toBe(true);
  });

  it("precedence: the capacity signals suppress the redundant top-off, and never double-fire each other", () => {
    const both = runAllRules(ctx({ txn: txn({ gallons: 150 }) })).map((r) => r.ruleId);
    expect(both).toContain("exceeds_tank_capacity");
    expect(both).not.toContain("exceeds_capacity_unverified");
  ...
  it("tank_space_exceeded and cumulative_overfuel reconcile against the sensor-resolved capacity", () => {
    ...
    const overEntered: VehicleView = {
      ...vehicle,
      tankCapacityGal: 300,
      sensorCapacityGal: 200,
      tankSensorReliable: true,
    };
    expect(
      ids(ctx({ vehicle: overEntered, txn: txn({ gallons: 160 }), tankPctBefore: 45 })),
    ).toContain("tank_space_exceeded");
