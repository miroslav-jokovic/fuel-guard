...
describe("who the carrier is", () => {
  ...
  it("prefers the organisation's own DOT number", () => {
    const carrier = resolveCarrierIdentity({ orgDotNumber: "1864495", envDotNumber: "43586" });
  ...
  it("falls back to the environment for an org that has not filled one in", () => {
    const carrier = resolveCarrierIdentity({ orgDotNumber: null, envDotNumber: "43586" });
  ...
  it("treats blank and whitespace as absent, not as an identity", () => {
    expect(resolveCarrierIdentity({ orgDotNumber: "   ", envDotNumber: "" }).source).toBe("none");
    expect(resolveCarrierIdentity({}).dotNumber).toBeNull();
  });

  it("carries a Motor Carrier ID when that is all the environment has", () => {
    const carrier = resolveCarrierIdentity({ envMotorCarrierId: "MC-1" });
...
describe("readiness is judged by the validator itself", () => {
  const carrier = resolveCarrierIdentity({ orgDotNumber: "1864495" });
  ...
  it("reports a missing carrier number once per driver, because it blocks all of them", () => {
    const none = resolveCarrierIdentity({});
    const { summary } = screeningReadiness(
