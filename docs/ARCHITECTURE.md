...
| `maintenance` (built 2026-08-27, D-SEP8; program step P5.3, `apps/api/src/modules/maintenance/`) | **Four tables today:** `maintenance_inspectors`, `vehicle_inspections`, `vehicle_inspection_items`, `maintenance_print_profiles` — the §396.17 annual inspection (D-S360-6, `docs/plans/maintenance/ANNUAL-INSPECTION-PLAN.md`), its first owned tables. The print-profiles hedge this row carried ("only if the office keeps printing onto pre-printed stock") is retired: A8 was ruled against, then reversed when the owner used the page — the truck copy IS printed onto the pre-printed pads, and 0283 shipped (that plan's §8 B5). **Ten more arrive with inventory** (D-S360-7, `docs/plans/maintenance/INVENTORY-PLAN.md`): `stock_locations`, `parts`, `part_stock`, `part_movements` (step I2), `stock_count_sessions` (I5), `asset_types`, `inventory_assets`, `asset_movements`, `kit_expectations` (I7), `inventory_settings` (I11). That plan's step text says "nine", counting the nine that land through I7; the tenth is I11's settings row on `idle_settings`' shape. Repair spend still arrives as McLeod AP via `financial` (`category='maintenance'`) — and stays the only door: the `fleetpal` collector is operational-only (D-INV10) and parts cost never reaches Finance (D-INV11). ⚠ The inspection feature writes NO table it does not own: the report PDF is filed through `evidence`'s interface, the expiry through `roster`'s (D-AVI10). Inventory holds the same line — equipment is read through `roster`'s interface (I9 adds `listEquipmentIdentities` there first), and no inventory table takes a `drivers` FK at all (D-INV3: an asset's holder is a unit, never a person). | maintenance |
