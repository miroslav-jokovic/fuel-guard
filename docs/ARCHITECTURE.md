...
| `fleetpal` | `apps/api/src/modules/fleetpal/` — **in build from 2026-09-10**, plan `docs/plans/maintenance/FLEETPAL-INTEGRATION-PLAN.md` (steps F0–F15, D-FP1–14); the module directory does not exist until F2 | FleetPal maintenance API (`openapi.fleetpal.io/v1`, bearer key) | planned: `fleetpal_credentials` · `fleetpal_sync_state` · `fleetpal_units` · `fleetpal_webhook_deliveries` (F2), then `fleetpal_work_orders` · `fleetpal_jobs` · `fleetpal_job_items` · `fleetpal_service_history` · `fleetpal_meters` · `fleetpal_pm_schedules` · `fleetpal_vendors` · `fleetpal_shops` (F6) and `fleetpal_defects` · `fleetpal_issues` · `fleetpal_expirations` (F7). **The collector writes no table it does not own** (D-FP2): stock and catalogue changes go through `modules/maintenance/inventory`'s exported `recordMovement` / `createPart`, and roster identity resolves inside `fleetpal_units` rather than as a column on `vehicles`/`trailers` (D-FP7). — **operational, not financial** (D-INV10, ruled 2026-09-08). What FleetPal owns is the repair job: work orders, PM schedules, DVIR defects, and what a job consumed. What it does NOT own is money. The financial contract this row carried from 2026-08-27 until 2026-09-08 — that a FleetPal work-order expense must project into `financial_entries` under a `maint:<vendor-invoice-or-wo-ref>` dedup_key computed identically for the McLeod AP voucher billing the same wrench — is **deleted**, not deferred: the 2026-09-03 fleet ruling (D-FLEET2, `docs/plans/financial/FINANCE-FLEET-REPORT-PLAN.md`) made `mcleod_gl_totals` × `mcleod_gl_accounts` the whole financial input, so there is no second door left for a dedup contract to guard. Parts cost does not arrive here either (D-INV11): GL `30230000 Shop Parts` already carries $270,670.22 inside a $1.42 M maintenance family tied to the printed income statement, and D-FS1's canonical index plus D-FLEET1/D-FLEET8 close the rest. D-SEP8's gate survives in its operational half — FleetPal still may not land a first row before its ingest contract is written, it is now an ownership contract against `maintenance` (the shelf is ours, the job is FleetPal's) rather than a dedup one against `financial`. **⚠ Per-unit repair COST is nevertheless built** (D-FP3, ruled 2026-09-10): FleetPal is the only source in the stack that knows what one truck cost to repair — `mcleod_gl_totals` has no equipment dimension at all — so `/v1/service-history` is staged and reported in the Maintenance section, **never in `financial_entries` and never in the fleet report**, and never without the coverage ratio that says what fraction of the GL maintenance family FleetPal actually saw (D-FP4). That is an operational number beside a completeness measure, not a second financial source, so D-FLEET1/D-FLEET2/D-FLEET8 are untouched. Plan: `docs/plans/maintenance/FLEETPAL-INTEGRATION-PLAN.md` (supersedes `INVENTORY-PLAN.md` §I14 as the queue; §2.7 there remains the ownership statement). |
...
| `maintenance` (built 2026-08-27, D-SEP8; program step P5.3, `apps/api/src/modules/maintenance/`) | **Four tables today:** `maintenance_inspectors`, `vehicle_inspections`, `vehicle_inspection_items`, `maintenance_print_profiles` — the §396.17 annual inspection (D-S360-6, `docs/plans/maintenance/ANNUAL-INSPECTION-PLAN.md`), its first owned tables. The print-profiles hedge this row carried ("only if the office keeps printing onto pre-printed stock") is retired: A8 was ruled against, then reversed when the owner used the page — the truck copy IS printed onto the pre-printed pads, and 0283 shipped (that plan's §8 B5). **Four of inventory's ten have ARRIVED** (D-S360-7, `docs/plans/maintenance/INVENTORY-PLAN.md`): `stock_locations`, `parts`, `part_stock` and `part_movements` shipped at step I2 (migration 0331, 2026-09-09), and step I3 mounted `/api/maintenance/inventory/…` over them, so the module owns **eight tables today**. `part_movements` is append-only by trigger (`IV011`) and is the ledger `part_stock.quantity_on_hand` projects from — `record_part_movement` is its only writer (D-INV4). Six still to come: `stock_count_sessions` (I5), `asset_types`, `inventory_assets`, `asset_movements`, `kit_expectations` (I7), `inventory_settings` (I11). That plan's step text says "nine", counting the nine that land through I7; the tenth is I11's settings row on `idle_settings`' shape. Repair spend still arrives as McLeod AP via `financial` (`category='maintenance'`) — and stays the only door: the `fleetpal` collector is operational-only (D-INV10) and parts cost never reaches Finance (D-INV11). ⚠ The inspection feature writes NO table it does not own: the report PDF is filed through `evidence`'s interface, the expiry through `roster`'s (D-AVI10). Inventory holds the same line — equipment is read through `roster`'s interface (I9 adds `listEquipmentIdentities` there first), and no inventory table takes a `drivers` FK at all (D-INV3: an asset's holder is a unit, never a person). | maintenance |
...
| File/function budgets | `lint:filesize` / `lint:funcsize` | live — **and the gap this row claimed is closed**, re-measured 2026-09-08 while writing INVENTORY-PLAN I0. `check-file-size.mjs` scans `SCAN_DIRS = ["apps", "packages"]` (`:57`) over `SOURCE_EXT = .ts, .tsx, .vue` (`:122`), so web `.vue`, driver `.tsx` and `packages/shared` are all inside the 500/450 budget; `.tsx` was added at the point it was found to be silently exempting all 68 driver UI files (that script's header, `:26`). `lint:funcsize` is still `apps/api/src` only, by design — a router factory is a flat list, not a function to budget. The inventory forms are extracted from the first commit because `.vue` counts. |
