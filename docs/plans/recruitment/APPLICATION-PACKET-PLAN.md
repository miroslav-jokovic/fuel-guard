...
| **D-PKT21** | **The packet's fines follow the driver handbook where both price the same offence** (owner, 2026-09-25: *"the handbook should win on the fines, fix the packet"*). Ten changes on pages 7, 9 and 10, each with its reason, in `packetFines.ts`; applied after D-PKT20's spelling, inside the carrier's own lines. An offence only one document prices is left as it is, and a packet rule with no figure is not given one. |
