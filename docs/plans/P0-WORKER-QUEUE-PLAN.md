...
| **Q11** | **Wake via LISTEN/NOTIFY + jittered poll fallback.** enqueue emits `pg_notify('jobs', kind)`; idle workers wake instantly; a 2–5 s jittered poll backstops missed notifies and drives the lease-expiry reclaim scan. | Low latency without busy-polling; no extra infrastructure. |
...
|-------|-------|-------|-------------|
| **WQ0** — Queue foundations | `jobs` schema evolution + claim/lease RPCs + worker claim loop + handler registry, behind `JOB_EXECUTION_MODE`, proven on ONE low-risk kind | ☐ | **CURRENT** |
