...
| **WQ2** — Hazmat → queue kinds | extraction/analysis become `hazmat_extract`/`hazmat_analyze`; semaphores → pool concurrency | ◐ extraction path done (`hazmat_extract`: executeExtraction extracted, semaphore inprocess-only, worker cap in queue mode; idempotent via existing-run check; per-load dedup deferred — unique runId key preserves today's no-dedup behavior) | **WQ2b: manual analysis (`hazmat_analyze`, hazmatAnalysis.ts, same pattern)** |
