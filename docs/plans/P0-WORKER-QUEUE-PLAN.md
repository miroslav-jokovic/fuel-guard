...
| **WQ1** — Migrate ledger kinds | request + scheduler call sites enqueue; per-kind handlers reconstruct from payload; dedup/progress/cancel preserved | ✅ all kinds migrated: scoring (WQ1) + `efs_soap_*` + manual `efs_ingest` (WQ1b) + **WQ1c: the Samsara sync kinds (`sync_vehicles`/`_trailers`/`_idle`/`_drivers`/`_driver_scores`/`_stats`), `snapshot_driver_week`, `nightly_reconcile`, and the manual `efs_soap` sync-now** — all via `dispatchJob`/`enqueueJob` + handlers (`handlers/samsara.ts`) + 7 tests; A2 held | → contract change delivered: the manual sync endpoints now return `202 {jobId}`; the 4 direct-toast pages (Vehicles/Drivers/Trailers/Driver-Performance) rewired to `useBackgroundSync` (ledger poll → toast real counts on completion), joining the DataSync cards |
| **WQ2** — Hazmat → queue kinds | extraction/analysis become `hazmat_extract`/`hazmat_analyze`; semaphores → pool concurrency | ✅ done — both paths (`hazmat_extract` + `hazmat_analyze`): execute\* bodies extracted, semaphores inprocess-only, worker per-kind caps in queue mode, idempotent existing-run guard | → **WQ3** |
| **WQ3** — Multi-replica + leases | lease-aware boot sweep (no longer fails leased/queued jobs); `WORKER_ROLE` scheduler/consumer split (1 scheduler + N consumers) | ✅ boot sweep + role split done; **Q7 landed with WQ1c — every vendor-calling kind pinned to a fleet-wide in-flight cap of 1 in the consumer `KIND_CAPS` (bounded lane); dedicated single-replica Samsara-lane consumer documented for high fan-out** | → **WQ4** (live validation + cutover) |
| **WQ4** — Cutover & cleanup | default `queue` mode; remove in-process path; update WORKER-DEPLOYMENT.md; job metrics + Sentry | ◐ **prep landed**: queue observability (`GET /api/jobs/queue-metrics` + passive `[queue-metrics]` scheduler log with the A1 300s backlog tripwire, `services/queue/metrics.ts` + tests) and the cutover runbook (`docs/WQ4-CUTOVER-RUNBOOK.md`) | **operator: apply 0095 → Phase A canary validation in queue mode → Phase B flip → Phase C code cleanup (own PR)**. The flip + live validation are environment actions, not a merge |
