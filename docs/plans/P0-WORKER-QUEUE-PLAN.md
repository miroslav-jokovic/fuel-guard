...
| **WQ1** — Migrate ledger kinds | request + scheduler call sites enqueue; per-kind handlers reconstruct from payload; dedup/progress/cancel preserved | ◐ all `runJob`-closure kinds migrated: scoring (WQ1) + `efs_soap_*` + manual `efs_ingest` (WQ1b), via `dispatchJob` + handlers + tests; A2 held | **WQ1c: `sync_*` (integrations.ts) + `nightly_reconcile`** — `sync_*` return results inline (`res.json`), so queueing them needs a client-contract change + web polling (own batch) |
...
| **WQ3** — Multi-replica + leases | lease-aware boot sweep (no longer fails leased/queued jobs); `WORKER_ROLE` scheduler/consumer split (1 scheduler + N consumers) | ◐ boot sweep + role split done (topology enforces single scheduler, so advisory-lock leader Q13 not needed) | **distributed Samsara/SOAP limits (Q7) rides on WQ1c queueing those kinds** |
