...
| ~~**`dbo.fuel_detail` — 3 rows**~~ **CORRECTED 2026-08-26** | schema catalog | ~~McLeod is not the fuel source of truth.~~ **This row read the live table and missed McLeod's live/history split: `fuel_detail_hist` has 65,847 rows and `fuel_ticket_hist` 78,213.** EFS stays authoritative for fuel *purchases*, but McLeod fuel is a real reconciliation input and the only source of `reefer_cost`/`def_cost` splits. See `mcleod/MCLEOD-CPM-DATA-SOURCE-SPEC.md` §2.1 (D-MC11). |
...
| **Fuel transactions** | ~~`fuel_detail` = 3 rows~~ → `fuel_detail_hist` **65,847**, `fuel_ticket_hist` **78,213** | **CORRECTED 2026-08-26.** The original verdict read only the live table. McLeod archives completed records to `_hist`, and the history tables are fully populated with `tractor_id`, `driver_id`, `movement_id`, `tractor_gals`, `reefer_cost` and `def_cost`. EFS stays authoritative for purchases; McLeod fuel is a reconciliation input and the source of the reefer/DEF split. See `mcleod/MCLEOD-CPM-DATA-SOURCE-SPEC.md` §2.1. |
...
| **Cost per mile** | GL `tractor` column present but **0 of 188,179 2026 lines populated**; subledgers 100% attributed (`drs_settle_hist` 20,833/20,833) | **Verdict stands, mechanism corrected 2026-08-26.** It is not that the cost data is missing — it is that the *general ledger* carries no equipment coding at this carrier, so attribution must come from the subledgers and the GL serves as a control total only. Extraction is implementable now; the allocation of unattributed cost is the part that needs finance sign-off. See `mcleod/MCLEOD-CPM-DATA-SOURCE-SPEC.md` §3 (D-MC12). |
