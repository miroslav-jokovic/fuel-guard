...
> Owner: Silvicom Inc. · Status: **BUILDING — Phases 0–1 built (foundation + identity/auth); scope pivoted to loads/assignments + planned navigation (D41); Phases 2–7 authored/roadmapped; every decision LOCKED (D1–D59)** · Last updated: 2026-07-28
>
> ✅ **Solutions-only.** Every architecture/design/security/UX/compliance choice is a **LOCKED decision
> (D1–D59)** with a rationale and a documented fallback. **There are no "decide during build" items left**
...
> D1–D59); §10 holds only operational pre-build *tasks* (no research required).
...
2. Read **§1 Progress Ledger** first — it says which phase is current and what the next action is
   ("you are here").
3. Read the **Locked Decisions (§9, D1–D59)** and the **operational pre-build checklist (§10)** before doing any work — they
...
8. Every design/architecture choice is already LOCKED (§9, D1–D59); §24 maps the former open questions
...
| 5N — Notifications | ✅ authored (D53) | ◐ **backend built** — **`0089_notifications.sql`**: `notification_events` / `notification_reads` / `device_push_tokens` / `notification_preferences`, a local **`auth_user_id()`** helper (so the offline matrix exercises the real predicate rather than Supabase's `auth.uid()` shim), per-user RLS where **even an admin cannot open another inbox**, `emit_notification()` applying module + mute + quiet-hours + **dedupe** in one place, and `revoke_push_tokens()`; shared `notificationsContract.ts` (suppression mirrored, 21 tests); `services/notify.ts` with Expo Push batching and `DeviceNotRegistered` token revocation; emission wired into **release** and **cancel**; tokens revoked on offboarding *and* account deletion | ◐ RLS **+17 cases**; shared 21 tests. **Driver-side bell/centre + `expo-notifications` registration pending a dev-client rebuild** |
| 5M — Messages | ✅ authored (D54) | ◐ **backend built** — **`0090_messages.sql`**: `message_threads` (**`load_id`-bound**) / `thread_participants` / `messages` (**client-UUID PK**, soft-delete only, hard delete refused by trigger) / `message_reports` (Apple 1.2 affordance); access by **participation, not org membership** via `auth_in_thread()`; `thread_unread_count()`; `messages` added to the realtime publication (guarded). API `/api/messages` for **both sides** — the audience is resolved server-side, never by the client, and `thread_participants` has no INSERT policy at all. Sending emits a `message_received` notification | ◐ RLS **+21 cases**. **Driver + dispatch screens pending** | `notification_events` / `notification_reads` / `device_push_tokens`, Expo Push, top-bar bell + centre, per-category prefs + quiet hours, deep links, token revocation on offboarding |
...
| **D59** | **Build hygiene is enforced by the toolchain, not remembered.** Four rules, each backed by a check rather than a convention: **(1) Expo runs only from `apps/driver`** — `pnpm driver:start` / `driver:ios` / `driver:android` from the root, never a bare `npx expo` at the workspace root, which prebuilds a second `com.anonymous.*` app into a root `ios/` + `app.json` that is gitignored and therefore survives every branch switch unnoticed. **(2) Metro watches only what the bundle can reach** — `watchFolders` is `packages/shared` + the hoisted root `node_modules`, and an explicit `blockList` excludes `ios/{Pods,build,DerivedData}`, `android/{build,.gradle,.cxx}` and cloud-sync conflict copies. Metro's default blockList excludes none of these and always watches the project root. **(3) `packages/shared/dist` can never be stale** — `start`/`ios`/`android` run `build:rn` first; `metro.config.js` throws when the dist is missing and warns when it is older than `src`. **(4) The checkout does not live in a cloud-synced folder** — a sync daemon racing a build writes ` 2`/` 3` conflict copies, which are haste collisions in source and a corrupt index in `.git`. **(5) A dev client is a BINARY, and adding a native dependency does not change the one on the phone.** `ios/Podfile.lock` is the manifest of what the last native build actually linked; any dependency shipping a `.podspec` that is absent from it means the app on the device cannot run the current bundle, and will die on `Cannot find native module` immediately after Metro serves it. **(6) Port 8081 must be free before `expo start`** — see the rationale. `pnpm driver:doctor` (`scripts/driver-doctor.mjs`) checks all six plus `node-linker=hoisted`, stale git locks, evicted iCloud files and duplicate `.podspec` copies, and exits non-zero. Supersedes the `watchFolders` → repo-root instruction in §11.4 | **The cause, established by bisection and not by inference: a leftover Metro process holding port 8081.** `expo start` prints *Port 8081 is running this app in another window* and asks *Use port 8082 instead?* — and that question comes **after** *Starting Metro Bundler*. In a normal terminal it is a prompt that waits forever, so the symptom is a build that stops on *Starting Metro Bundler* with no error, and it survives reinstalls, cache clears, watchman and every config change, because the process holding the port is simply still running. The zombie came from the stray root project (rule 1) being started at the workspace root. Rules 2–4 were real problems found while hunting it — the crawl, the stale dist, the sync-conflict copies — but **none of them was the hang**; with the blockList in place Metro starts in 3 seconds, and it started in 3 seconds without it too. Recorded this way deliberately: the first version of this row asserted the crawl was the cause, on reasoning rather than measurement, and it was wrong |
...
**Every design/architecture/security/UX/compliance choice is now a LOCKED decision (D1–D59).** What
...
- `metro.config.js`: enable monorepo node-modules resolution; ensure the transformer compiles TS from
  the workspace package; validate the `.js`-suffixed ESM specifiers + `moduleResolution: bundler`
  resolve by importing a pure function. **`watchFolders` is `packages/shared` + the hoisted root
  `node_modules` — not the repo root (D59).** Watching the repo root put `apps/web`, `TemplatesTailwind`,
  `.pnpm-store`, `data-samples` and a 1.13 GB `ios/Pods` tree into Metro's crawl and hung the first
  device build; the `blockList` in D59 covers the native output that lives *inside* the project root.
...
|---|---|---|---|---|
| 2026-07-28 | **iOS rebuild blocked — codesign vs. iCloud extended attributes** | *(working tree)* | `driver-doctor` checks 13–14 | The dev-client rebuild failed inside `[CP-User] Build ExpoModulesJSI xcframework` with `ExpoModulesJSI.framework: resource fork, Finder information, or similar detritus not allowed`. **That is codesign refusing extended attributes, not a bug in expo-modules-jsi** — since iOS 10 / macOS Sierra no file in a signed bundle may carry a resource fork or Finder info (Apple QA1940), and a cloud file provider stamps exactly those onto everything it touches. The error names a framework, so it reads as a native-module problem and sends you into the wrong library. **This is the point where the synced checkout stops being hygiene and becomes a hard blocker:** the same cause has now produced conflict copies of `.git/index`, duplicate `.podspec` files that CocoaPods would double-link, files evicted to the cloud that block on read, and now a code signature that cannot be produced. `xattr -cr` strips `FinderInfo`/`ResourceFork`; it does **not** strip `com.apple.provenance`, and if that is present the only remedy is to build off the synced path. **Second-order trap fixed at the same time:** the ExpoModulesJSI script caches into `node_modules/expo-modules-jsi/apple/.DerivedData` (207 MB), outside anything `expo prebuild --clean` or an Xcode clean touches — a failed run leaves a half-written framework that the next run reuses and fails on identically |
| 2026-07-28 | **Device crash — stale dev client (D59 rule 5)** | *(working tree)* | `driver-doctor` now names the three missing modules and the build date | Metro served the bundle, the app loaded it and died. **Cause: the dev client on the device was built 2026-07-25 17:12, before Phase 2 added the offline spine.** Its `Podfile.lock` links 19 Expo pods and does **not** contain `ExpoSQLite`, `react-native-netinfo` or `RNSVG` — all three are imported by the current bundle, and `src/data/db.ts` (the encrypted outbox) evaluates `expo-sqlite` during startup, so the failure lands in the first moment after load. Nothing was wrong with the code, the config or Metro. **Checked and cleared while hunting it:** Reanimated 4.5.0 / Worklets 0.10.0 / RN 0.86 / safe-area-context 5.7.0 / screens 4.26.2 all match both the SDK-57 pins and the versions in the built `Podfile.lock`, so there is no JS↔native version mismatch; `node-linker` really is `hoisted`; one copy each of zod, react, react-native, reanimated; pnpm is 10.34.4, so the pnpm-11 `.npmrc` regression does not apply. **Also found and cleared:** 10 sync-conflict `.podspec` duplicates inside `node_modules` (`RNReanimated 2.podspec`, `Expo 2.podspec`, …), which CocoaPods can link as a second copy of the same pod. **Deferred, deliberately not changed in the same step as the rebuild:** `babel.config.js` lists `react-native-worklets/plugin` manually although `babel-preset-expo` adds it automatically, and eight driver dependencies use `"*"` or `^` where Expo pins exact or `~` |
| 2026-07-28 | **Build unblocked — root cause found (D59)** | *(working tree)* | `scripts/metro-bisect.sh`: 11 variants, L8 (`pnpm run start` verbatim) **UP after 3s** | **A leftover Metro on port 8081.** Found by bisection after the theory-driven fixes failed: eleven variants of `expo start` — with and without our metro.config, cleared and warm cache, interactive and not, online and offline, ports 8081 and 8091 — every one came up in **3 seconds**, which disproved the crawl theory outright. The variant that finally exposed it was the *non-interactive* one, which prints `Port 8081 is running this app in another window … Use port 8082 instead?` and exits instead of blocking. In an ordinary terminal that same question is an unanswered prompt, printed after `Starting Metro Bundler`, and it looks exactly like a hang. **The lesson, kept in the repo rather than in a commit message:** `driver-doctor` check 10 is now a **blocking** failure with the pid and the kill command, and `metro-bisect.sh` frees the port by port rather than by process name — killing `pnpm run start` leaves its node grandchild listening, which is how the harness poisoned its own next variant and re-demonstrated the bug |
| 2026-07-28 | **Build unblock — driver toolchain hardening (D59)** | *(working tree)* | `pnpm driver:doctor` exits 0 on a clean tree; `node --check` clean on both new scripts | First device build hung with no error. **Three causes, all silent:** a stray Expo prebuild at the workspace root (`app.json`, `ios/`, `.expo/`, `tsconfig.json` — gitignored, pod install stopped at 4 of 15 pods, 2.8 MB `xcodebuild.log` with an *empty* error log); `watchFolders = [workspaceRoot]` with no `blockList`, so Metro crawled 1.9 GB of `node_modules`, 1.13 GB of `apps/driver/ios`, `TemplatesTailwind`, `.pnpm-store`, `apps/web` and `data-samples` before bundling a byte; and `packages/shared/dist` two hours stale — 14 source files newer, and `dist/index.js` missing the `entitlements`/`notificationsContract`/`messagesContract` exports entirely. **Fixed:** `metro.config.js` rewritten (narrowed `watchFolders`, blockList, hard error on a missing dist + warning on a stale one), `start`/`ios`/`android` now run `build:rn` first, new `scripts/driver-doctor.mjs` preflight, stray root project moved to `_to_delete/`. **Environmental, not fixable in-repo:** the checkout sits in a cloud-synced folder — found `.git/index 2`…`index 8`, `.npmrc 2/3`, `Podfile 2`, `app 2.json`; the same mechanism behind the 192 duplicate source files and the files that read as missing mid-session. The checkout has to move off the synced path |
| 2026-07-27 | **5N Notifications + 5M Messages (backends)** | *(working tree)* | RLS **153/153** (+38) · shared **907/907** (+21) · API **159/159** · web vue-tsc 0 errors · duty 20/20 · lifecycle 42/42 | `0089_notifications.sql`, `0090_messages.sql`, `notificationsContract.ts` + tests, `messagesContract.ts`, `services/{notify,messages}.ts`, `routes/{notifications,messages}.ts`, emission on release/cancel, revocation on offboarding. **Two things the tests forced:** `auth.uid()` replaced with a local `auth_user_id()` so the matrix tests the real predicate, and a reporter given SELECT on their own report — under RLS an `INSERT … RETURNING` must satisfy the SELECT policy, so reporting would have failed the moment a client asked for the row back |
...
decisions §9 (D1–D59, plus the §9.1 cleanup record); the audit sections §20–§24 are historical provenance (§9 governs where
