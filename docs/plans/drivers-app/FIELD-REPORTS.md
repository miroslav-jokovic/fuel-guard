...
| --- | --- | --- | --- |
| FR1 | **Safe-area problems on Android.** Content is not respecting the system insets. | **FIXED 2026-08-07.** One missing line, nine screens. `Screen`'s `padTop` prop gated BOTH the 16pt visual gutter and the safe-area inset, and nine screens pass `padTop={false}` because each renders its own `ScreenHeader` and means "my header owns the top spacing" — so the status-bar clearance went out with the gutter. Under `edgeToEdgeEnabled` the app draws behind the system bars, so every header except the tabs sat under the clock: settings, gallery, stop capture, hazmat, both message screens, notifications, check-in, end-shift. `padTop` now controls the gutter only; the inset is unconditional. | Fixed |
