...
| Privacy manifest with collected data types | ✅ **verified in a SHIPPED ARTIFACT 2026-09-08** — the first iOS build (`1.0.0` build 1, EAS `0ec2b351`) was downloaded and unpacked: `PrivacyInfo.xcprivacy` present with **7 collected types, 3 required-reason APIs, tracking false**. Previously this row rested on a `prebuild` output, which is what the build INPUT looks like rather than what Apple receives |
