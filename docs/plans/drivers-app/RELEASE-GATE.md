...
| Privacy manifest with collected data types | ✅ **2026-09-07, corrected 2026-09-08** (§6 P1) — seven collected types, **four** required-reason APIs, no tracking, no location. ⚠ This row said ✅ on three APIs and was wrong, and the way it was wrong is the lesson: `PrivacyInfo.xcprivacy` had been "verified in an `expo prebuild` output", which proves the manifest is PRESENT and says nothing about whether it is COMPLETE. The binary is the only thing that knows — `nm -u libExpoSQLite.a` answers `_fstatfs`, so SQLCipher needed `NSPrivacyAccessedAPICategoryDiskSpace` (85F4.1) and no expo package ships a manifest declaring it. Missing, that is an **ITMS-91053** upload failure. Re-verify the same way after any dependency change: `nm -u $(find "$DERIVED_DATA" -name '*.a') \| grep -E '_f?statfs\|_getattrlist\|activeInputModes'`  **Re-verified against a SHIPPED ARTIFACT 2026-09-08** (#679): build `1.0.0 (1)`, EAS `0ec2b351`, was downloaded and unpacked — `PrivacyInfo.xcprivacy` present, 7 collected types, tracking false, and **three** required-reason APIs, because that build predates the `NSPrivacyAccessedAPICategoryDiskSpace` declaration above. The artifact is the right verifier and this is what it says; the row stays at four because `app.config.ts` now declares four, and the next build is the one that will show them |
| Export compliance (`ITSAppUsesNonExemptEncryption`) | ✅ **declared true 2026-09-08** — was `false` on the grounds of "HTTPS + OS crypto", which ignored that `expo-sqlite` is built `useSQLCipher: true` and `aes-js` is bundled. AES-256 carried in the app is not the "encryption within the Apple operating system" Apple's table exempts, and it fits no Category 5 Part 2 exemption. Expect App Store Connect to ask export questions on every submission until a compliance code exists. ⚠ **Q-PR8 is open for counsel**: the French declaration and whether the 5D992.c self-classification report to BIS applies. Neither blocks a TestFlight build |
| iOS build number increments | ✅ **2026-09-08** — `appVersionSource: "remote"` + `production.autoIncrement`, so EAS owns the counter (Q-PR9). It was `local`, sourced from `IOS_BUILD_NUMBER`, which is set ONLY in `driver-store.yml` — a workflow that has never executed. Both of 2026-09-08's laptop-cut production builds were stamped `1`, and App Store Connect refuses a `CFBundleVersion` it has already accepted |
| Apple Developer Program membership type | ⚠ **Individual — the open launch blocker.** Guideline **5.1.1(ix)**: apps "that require sensitive user information should be submitted by a legal entity that provides the services, and not by an individual developer". This app carries driver identity and §391.51 qualification records. "Should", and trucking is not on Apple's example list, so it is a risk rather than a certainty — but it is a rejection risk, on top of the two reasons Q-PR7 already recorded (Individual admits one TestFlight seat; the seller name reads "Miroslav Jokovic", not Silvicom Inc). Organization conversion needs a D-U-N-S number and takes days to weeks — **start it before it is on the critical path** |
| Updated age-rating questionnaire | ☐ **owner action, App Store Connect** — deadline was **2026-01-31**; accounts that have not answered are blocked from submitting updates. Not visible from the repo, so it cannot be gated here — check it before the first submission, not during |
| DSA trader status | ☐ **owner action, App Store Connect** — required since **2025-02-17** and required **globally**, not only for EU distribution. Apps without it are removed from sale across all 27 EU storefronts, automatically and regardless of whether the app changed |
...
| Push permission | requested only when the `notifications` feature is on. ⚠ The bundle identifier carried **no capabilities at all** until 2026-09-08 — Xcode had auto-created it and `PUSH_NOTIFICATIONS` was added via the App Store Connect API that day. Without it a push token would never have minted, and nothing would have failed loudly |
| Store build drops dev tooling · APNs production · ATS closed (D-PR10) | ✅ **verified in the shipped artifact 2026-09-08** — no dev-menu or dev-launcher bundle in `Silvicom360.app`; entitlements say `aps-environment: production`; `NSAllowsArbitraryLoads` and `NSAllowsLocalNetworking` both false. The `APP_VARIANT=store` branch had never been exercised on a real build before this one |
