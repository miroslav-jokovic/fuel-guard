...
`auth_driver_id()` while the record itself is retained. `load-lifecycle` moved 42 → 54 with H-C1:
the hazmat↔dispatch load link, its supersede-chain hand-off, and the composite foreign key that makes
a cross-tenant link impossible rather than merely unlikely. `hazmat_rls` and `duty-sessions` are
unchanged.
