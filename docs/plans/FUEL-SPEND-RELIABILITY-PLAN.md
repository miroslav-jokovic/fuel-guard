...
| **Q-FX8** | ~~Who owns an exception operationally — fleet manager, controller, or a new role? Decides the default assignee and whether a read-only controller view is needed.~~ **ANSWERED 2026-09-06 with Q-FUI4, which inherited it** (`docs/plans/fuel/FUEL-SECTION-CONSOLIDATION-PLAN.md` §6). **No new role, and no default assignee** — a finding is claimed from the queue and aging makes an unclaimed one visible. The write gate is not `fuel` either: it is derived per finding from the kind's OWN section (`findingAssignment.ts`), because the merged inbox holds `safety` findings whose owner is the safety manager. | Miki | ~~open~~ Answered with Q-FUI4. |
