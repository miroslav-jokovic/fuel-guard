...
| **Q-FUI4** | ~~Inherits **Q-FX8** from `FUEL-SPEND-RELIABILITY-PLAN.md` §6 — who owns a finding operationally. C7b needs a default assignee.~~ **ANSWERED 2026-09-06 (owner ruling): the write gate is derived from the FINDING'S OWN SECTION, and there is no default assignee.** ⚠ The recorded fallback could not be taken as written: `rolesThatManage("fuel")` is exactly `admin, fleet_manager`, and Q-FUI1 had since ruled that the merged inbox holds both sources with **each kind carrying its own section**. A uniform fuel gate would therefore have taken the theft queue away from `safety_manager`, who holds `safety: "manage"`, `fuel: "view"`, and works it on `/anomalies` today — a narrowing in passing, which is precisely what T2 refused to do to Q-FUI12's four routes. Built as `packages/shared/src/findingAssignment.ts`: `FINDING_SECTIONS` is total over every kind, so a new kind cannot be added without deciding who owns it, and `rolesThatManageFinding` derives from `SECTION_ACCESS` rather than listing roles. **No default assignee** — a finding is claimed, and aging is what makes an unclaimed one visible; a name written at creation that nobody agreed to is not ownership. ⚠ This changes no gate TODAY: `fuel_exceptions` can only hold the eight fuel-section kinds, so `requireSection("fuel")` already equals the derivation. It becomes load-bearing the moment C7b puts a `theft_case` in the same queue. | Miki | ~~open~~ Answered and built. |
...
| **Q-FUI11** | ⚠ **RE-ASKED 2026-09-06 — the original question is answered by events and the new one is narrower. `cumulative_overfuel` carries two thirds of every false positive in the queue. Does it keep accusing?** The old question asked in what ORDER the three root causes get fixed. Two of the three now are: the odometer cluster (#584 tank allowance + the `robustWindowMiles` correction) and card→truck attribution (Q-FUI16 + 0317's lease, now auto-explaining 105 of `card_multi_vehicle`'s 132 fires). The third — entered tank capacity — is **not a collection problem and SAM-S6 proved it**: the backfill took per-fill corroboration from 17.6% to 89.0% and the entered-vs-learned disagreement rate did not move (~70%, on 177 trucks now instead of 145). And the gate the old recommendation wanted is still the wrong lever, for the reason recorded below. **What is left is one rule.** Measured 2026-09-06: `cumulative_overfuel` has **67 live fires, 64 false positives, and one 'confirmed' that §8 already showed does not survive contact** (unit 742, a ceiling built on a 10.71 MPG baseline against a truck that reads 7.48 today). It is **64 of the 95 false positives in the whole queue — 67%**. Precision is 3/98 = **3.1%** with it and 2/33 = **6.1%** without. Its weight is **75**, above the 60 that makes a signal raise a review ON ITS OWN, and **52 of its 67 cases are that lone signal — 51 of the 52 dispositioned false**. Corroboration does not rescue it: of the 14 cases where a second axis agrees, **0 are confirmed and 13 are false**. Candidates: **(a)** re-weight to **0** and re-label as data quality — the shape this repo already chose for exactly this situation in `odometer_entry_suspect` (*"low severity, zero theft weight, so it never inflates a correlated case"*), so the rule keeps detecting and stops accusing (**recommended**: it is a number in `catalog.yaml`, fully reversible, it respects §8's objection to switching the rule off because the detector survives and stays visible, and it is the only candidate whose effect on the figure C7 is blocked on is MEASURED rather than hoped for — 95 false positives become 31); **(b)** lower it below 60 instead of to 0, keeping its corroborating value — rejected on the evidence above, because 13 of the 14 corroborated cases are false and none is confirmed, so corroboration buys nothing; **(c)** switch it off entirely — §8's argument against stands: zero confirmations at n=6 is untested rather than disproven, and at 1–1.5 fires per 1,000 fills since June it costs almost nothing to keep looking; **(d)** wait for the `SCORING_VERSION` 2 sweep — **11 of its 67 have already been re-scored under the current ruleset and survived**, so waiting is not a plan. ⚠ Whichever is chosen, it does NOT make C7 ship on its own: 6.1% is better than 3.1% and is still not a queue worth merging an inbox for. C7's own text is the standard — *the reward for a queue worth working, not the remedy for one that is not*. | Miki | ~~open~~ **ANSWERED 2026-09-06 (owner ruling): (a).** `cumulative_overfuel` is weight **0** (`catalog.yaml`, ruleset `fccfacce9d80`) and `SCORING_VERSION` is 3, which is what retires the 64 accusations already on the queue. ⚠ It does NOT unblock C7, which was recorded before the ruling and still holds. ⚠ And it went further than the recommendation described — see **Q-FUI17**. |
...
| **Q-FUI15** | **An owner filter on the ledger needs a member directory the ledger's readers may not read.** P3 wired `assignedTo` and shipped `?owner=me`, which needs nobody's list. A picker — "assigned to Sarah" — needs the org's members, and `GET /api/members` is `requireRole("admin")`: the accountant, the auditor and the dispatcher who live in this ledger would see an empty menu. Candidates: **(a)** a NAMES-ONLY directory read gated on `requireOrg` — id, display name, role, nothing else — which is what `lib/memberLabels` already assembles for the actor names printed all over the product (**recommended**: the names are already on screen; refusing the list while printing its contents is a boundary that protects nothing); **(b)** widen `/api/members` to the section-view set, which also exposes email and invitation state and is a real widening; **(c)** leave it at "mine", and let C7b decide when the inbox needs assignment. ⚠ This is Q-FUI4's operational twin: that one asks WHO owns a finding, this one asks who may be shown the list of candidates. | Miki | ~~open~~ **ANSWERED 2026-09-06 (owner ruling): narrower than the recommended (a).** Not a names-only org DIRECTORY but a per-section **assignable-people** read — `GET /api/fueling/exceptions/assignees?section=…` answers *who could be assigned a finding in this section*, and the answer is `rolesThatManageFinding`, the same derivation that decides who may CLOSE one. That identity is the design: offering somebody who could not then close it is a menu whose only product is a stuck finding. It is narrower than (a) in the way that matters — a `driver` holds `none` on every section and so appears in no list, without anybody writing "except drivers" anywhere, whereas a general `requireOrg` directory would have let one enumerate the office. `?section=` is validated against `FINDING_ASSIGNABLE_SECTIONS`, derived from the kind map, so it cannot enumerate a section the inbox holds no findings for; the caller must pass `canViewSection` for it; and the response carries **id, name, role** only — no email, by omission rather than redaction (Q-SAM7's shape). ⚠ `name` is nullable and stays that way: measured 2026-09-06, one of this carrier's four admins has never set a profile name, and the honest answer is a null the caller labels rather than a fallback to the address this route refuses to carry. ⚠ Two gates required a written argument — `routeGateLedger` (the section is a parameter, so no static `requireSection` exists to see) and `routeGates` (a static fuel gate would be the WRONG gate, not a missing one). |
...
| **Q-FUI17** | **NEW, 2026-09-06, raised by shipping Q-FUI11's answer.** **A weight-0 rule leaves no trace at all, and the ruling was taken on the understanding that it would leave one.** Candidate (a) was recommended, and chosen, on the argument that it is *not* candidate (c) — the rule keeps detecting and stops accusing, so the detector survives and stays visible. The first half is true of the ENGINE and false of everything persisted from it: `correlateSignals` filters `weight > 0` before building `signals`, and `persist.ts` writes that same list to `fuel_transactions.case_signals`. So `cumulative_overfuel` now produces no case, no signal on the fill, and nothing for `explainCaseOutcome` to mention — and `odometer_entry_suspect` has been in that position, silently, since it was written to be *"low severity, zero theft weight, so it never inflates a correlated case"*, which reads as an intention to record it and not to erase it. Candidates: **(a)** carry fired weight-0 rules in a separate unscored field on the assessment and persist them beside `case_signals`, leaving every scoring path byte-identical (**recommended**: it is additive, it delivers what the Q-FUI11 ruling was actually approved on, and it costs one field); **(b)** admit weight 0 means OFF, and say so in `catalog.yaml` next to both rules — cheap, honest, and it makes (a) and (c) the same candidate for every future ruling, which is a real loss because a weight is reversible and a deletion is not; **(c)** leave it undecided — rejected, because the queue is now being read as evidence and "the rule no longer fires" and "the rule fires and we discard it" are different facts about the fleet. ⚠ Not bundled into the Q-FUI11 merge on purpose: it changes shared correlation behaviour for a second rule that nobody has ruled on. | Miki | ~~open~~ **ANSWERED 2026-09-06 (owner ruling): (a).** `CaseAssessment.unscoredSignals` carries them and migration 0323 persists them in `fuel_transactions.case_signals_unscored`, apart from `case_signals` so `entityRisk` and every other existing reader is byte-for-byte unaffected. ⚠ It turned out to be a CORRECTNESS fix, not a display one — see §8. |
...
- 2026-09-06 · **Q-FUI11 re-asked, and the original is answered by events rather than by a ruling.**
  It asked in what ORDER the three root causes of the 2.9% get fixed, and whether the missing capacity
  gate ships first. Two of the three are now fixed and the third has been shown not to be the kind of
  problem that question assumed.

  **Odometer quality — fixed.** `robustWindowMiles` stopped measuring between the readings inside a
  window, and #584 stopped `expected_odometer_band` assuming the tank starts empty (430 production
  fires to 22). Neither is yet visible in the queue: the sweep does never-stamped fills first, and
  every one of the 19 `expected_odometer_band` and 16 `odometer_daily_cap` cases sits on a
  `scoring_version = 1` fill, which the sweep reaches last. **0 of 35 have been re-scored.**

  **Card→truck attribution — fixed.** Q-FUI16's fallback and 0317's lease let the auto-clear run for
  the first time since 2026-08-09. `card_multi_vehicle` went from 48 fires / 25 false positives to 132
  fires of which **105 are auto-explained** — 93 by the fill's own driver attribution, 6 by Samsara's.

  **Capacity — not a collection problem, and SAM-S6 settled it.** The backfill took per-fill
  corroboration from 17.6% to 89.0% of tractor fills and the entered-vs-learned disagreement rate did
  not move: ~70% before on 145 trucks, ~68% now on 177. More measurements of a defect that lives in
  the ENTERED data. And the gate Q-FUI11 originally recommended remains aimed at the wrong lever for
  the reason recorded on 2026-09-05: where the sensor reads BELOW entered, `resolveCapacity` already
  returns the entered value, which makes the over-fuel ceiling larger and the rule fire LESS.

  **So one rule is what is left, and it is most of the problem.** `cumulative_overfuel` is **64 of the
  95 false positives in the queue — 67% of the whole pool**. Precision reads 3/98 = 3.1% with it and
  2/33 = 6.1% without. The mechanism is its weight: **75, against the 60 at which a signal raises a
  review on its own**, so it accuses unaccompanied — **52 of its 67 cases are a lone signal, and 51 of
  those 52 are dispositioned false**. Corroboration does not rescue it either: where a second axis
  agrees, **0 of 14 are confirmed and 13 are false**. Its one lifetime confirmation is the unit-742
  case §8 already took apart.

  Its fire rate has also collapsed without anyone touching it — 17.9 per 1,000 fills in February, 10.5
  in March, then 0.5 to 1.5 from June onward and 0 so far in September. A rule that is both rare and
  wrong is a different problem from one that is common and wrong, and it is why (c) — switching it off
  — is a candidate rather than the answer.

  ⚠ **And whichever way it is answered, C7 still does not ship.** 6.1% is better than 3.1% and is not a
  queue worth merging an inbox for. C7's own text is the standard: *the reward for a queue worth
  working, not the remedy for one that is not.* Recording that here so the re-ask is not mistaken for
  a route to C7.

  ⚠ **78 of the 282 live cases have never been reviewed, and 55 of those 78 carry one of the two rules
  the `tankSensor` gate covers** — `tank_space_exceeded` and `tank_fill_short`, which between them have
  produced **5 false positives across 91 fires**. Every precision figure on this page is measured on
  the 105 cases that have been looked at. That is not a reason to distrust them — the false-positive
  count has been 95 across two independent readings a week apart — but the unreviewed backlog is
  concentrated in the two best-behaved rules in the section, so the queue may read BETTER after review
  rather than worse, and nobody should answer this question expecting the opposite.

- 2026-09-06 · **Q-FUI11 answered (a) and shipped: `cumulative_overfuel` is weight 0.** The owner ruled
  on the re-asked question the same day it was put. The change is one number in `catalog.yaml`
  (ruleset `fccfacce9d80`) plus `SCORING_VERSION` 2 → 3, and the second half is the load-bearing one:
  a weight change alters what scoring concludes from unchanged inputs, so without the bump the 64
  accusations already on the queue would keep a verdict the product no longer makes.

  **What it does, measured before it was written.** The rule was 75, above the **60** at which a signal
  raises a review UNACCOMPANIED — and unaccompanied is how it worked: **52 of its 67 live cases were a
  lone signal and 51 of those 52 were dispositioned false.** It carried **64 of the queue's 95 false
  positives**. Corroboration did not rescue it — 0 of 14 confirmed where a second axis agreed, 13
  false. Its body is untouched and still runs; only its authority to accuse is gone, and restoring it
  is the same one number.

  ⚠ **It went further than the recommendation described, and that is Q-FUI17.** (a) was recommended —
  and chosen — on the argument that it is *not* (c): the rule keeps detecting and stops accusing. That
  is true of the engine and false of everything persisted from it. `correlateSignals` filters
  `weight > 0` before building `signals` and `persist.ts` writes that list to `case_signals`, so the
  rule now leaves **no case, no signal on the fill, and nothing for `explainCaseOutcome` to say**.
  `odometer_entry_suspect` has been in that position since it was written, silently.

  It was not bundled into this merge: fixing it changes shared correlation behaviour for a second rule
  nobody has ruled on. What ships instead is the gap made impossible to miss — `records nothing at all
  for a weight-0 rule, which is Q-FUI17 and not the intent` asserts today's behaviour rather than the
  wanted one, and is the test that should fail, loudly and in the right file, the day Q-FUI17 lands.

  **What it does not do, recorded before the ruling and unchanged by it.** C7 does not ship. Removing
  `cumulative_overfuel` from the pool takes precision from 3.1% to a measured 6.1%, and 6.1% is not a
  queue worth merging an inbox for — C7's own standard.

  **Proved able to fail by two mutations of the catalogue.** Restoring weight 75 fails three cases.
  ⚠ More usefully, **weight 59 — below the review line but still scoring, which is candidate (b)** —
  fails two: the suite distinguishes the ruling that was made from the one that was rejected, rather
  than merely noticing that some number changed.

- 2026-09-06 · **Q-FUI17 answered (a) and shipped in two merges — and it was a correctness fix, not
  the display fix it was raised as.** `CaseAssessment` gains `unscoredSignals`, migration 0323 gives
  `fuel_transactions` a `case_signals_unscored` column, and the Fuel log's "why" panel reads both.
  Every scoring path is byte-identical: `signals` is still the weight-filtered list, computed from the
  same partition, and `level`, `score`, `severity` and `axes` never see the weightless rules.

  **The column is separate because `case_signals` has a reader that counts it.** `entityRisk` tallies
  that column per rule to rank trucks and drivers, so folding weightless rules in would move a truck up
  a risk list on the strength of a signal the product had just decided carries nothing — the exact
  confusion Q-FUI11's ruling exists to remove. Read at the call sites rather than assumed.

  ⚠ **The half nobody saw coming.** `contaminatesBaseline` tests `VOLUME_AXIS_RULE_IDS`, and that set
  contains `cumulative_overfuel`: a fill carrying that signal is EXCLUDED from the rolling window that
  trains `effectiveBaseline`. When the rule went to weight 0 the day before, it fell out of
  `case_signals` — and over-fuelled fills silently began training the very baseline they had always
  been kept out of. **A weight ruling changed what the MPG learner is fed, and nothing in the ruling
  said so.** It was small when found (33 of 16,256 fills at `SCORING_VERSION` 3) and grew with every
  night of the sweep, which is why 0323 did not wait. `consumptionContext` now passes both lists, and
  `still contaminates the MPG baseline, which weight 0 had silently stopped it doing` holds it.

  ⚠ **And a lesson about markers, because the one written for this failed to fail.** #587 shipped a
  case called *"records nothing at all for a weight-0 rule, which is Q-FUI17 and not the intent"*,
  described in its own PR as the test that should fail "loudly, and in the right file, the day Q-FUI17
  is answered". It did not fail. It asserted that `signals` is empty for a weight-0 rule — true before
  the fix, true after it, and true forever, because `signals` is the SCORED list and the fix put the
  weightless ones somewhere else. **A marker for a known gap must assert the thing that will CHANGE,
  not a thing that happens to be true while the gap exists.** Recorded rather than quietly replaced:
  it is a cheap mistake to make and an expensive one to notice.
