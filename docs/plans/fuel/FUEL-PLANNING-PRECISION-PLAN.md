...
- 2026-09-10 — **FP4 BUILT** (#730): readers of the 0335 columns; reserve is a share of the tank; `usableFraction` deleted; `BORDER_TOP_OFF_PCT` deleted; the form schema round-trips the five fields. Held until `pnpm verify:live` reported 0335 current.
- 2026-09-10 — **FP5 BUILT** (#731): pump price and your price on every stop with basis and age; `planPricing` totals; the stop view moved to `planStopView.ts` (the orchestrator crossed 500 lines).
- 2026-09-10 — **FP6 BUILT** (#732): the settings page in the planner's rule order; six fields retired from the form and `RouteFuelSettings`; `FuelPolicy.alwaysFillFull` and the Buy-discipline callout retired with min-drawdown.
- 2026-09-10 — **FP7 BUILT** (#733): `PLAN_FLAG_COPY` typed over every flag, rendered as notes under the banner; one speed (the route's) for the reachable-now tile and the walk; `savingsVsNaive`, `hos_limited`, `isMinFill` and the naive baseline run deleted; the fourth tile is the discount.
