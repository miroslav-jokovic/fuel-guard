...
| — | ⚠ **F9 / §7 Q6 re-run owed.** The first probe sent `Hold` to an account storing `ACTIVE` and reproduced H1 instead of measuring the override freeze. **Lesson worth carrying: `variant: "standard"` makes an experiment's EDIT ALGEBRA match production, not its VALUES** — the endpoint sends `status` verbatim on purpose. Pass `matchAccountCasing: true` for any status write meant to be production-equivalent | `docs/22` H16 |
