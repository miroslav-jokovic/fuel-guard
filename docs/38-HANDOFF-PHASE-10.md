...
| ✅ | ~~`deleteOverride` entitlement unconfirmed~~ — **CLOSED 2026-08-17**: entitled on this account and it lands (docs/22 H16) | `docs/22` H16 |
| ✅ | ~~F9 / §7 Q6~~ — **ANSWERED 2026-08-17: the freeze is REAL.** Two byte-identical `HOLD` writes, same card, same session: landed at `overrideUses: 0`, did NOT land at `1`. Field-scoped though — the override trio stays writable, `status` does not | `docs/22` H16 |
| ⚠ **NEW, and it outranks Phase 10** | **The safety fix F9 created.** No capability checks `overrideUses`, so `card_lock` — the 2am stolen-card action, deliberately frictionless — silently does nothing on a card with an exception, and the vendor returns a void success. Three options and a recommendation (B for lock, A for the rest) are in `docs/28` §10. **Scope is Miki's** | `docs/28` §10 |
| — | **Lesson worth carrying: `variant: "standard"` makes an experiment's EDIT ALGEBRA match production, not its VALUES** — the endpoint sends `status` verbatim on purpose. Pass `matchAccountCasing: true` for any status write meant to be production-equivalent, and give a drill a CONTROL whenever its negative result has more than one available explanation | `docs/22` H16 |
