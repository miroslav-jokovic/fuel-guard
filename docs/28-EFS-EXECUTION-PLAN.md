...
| 8 | Card status *(first production promotion)* | 🔶 **8.1 DONE · 8.2 PARTIAL — 2 of 3 proven on QA** *(PR #75, `46ee6cf`; migration **0199** applied — `Apply Supabase migrations` ✅ and the ledger reports `0199 | 0199 | 0199`. `card_deactivate` built as contract + behaviour + view and **proven on its first live run** (proof `6f28e05d`, 717 ms, wrote the account's own `INACTIVE`); `card_lock` re-proven (proof `88576345`, 514 ms). `card_lock` narrowed to `Hold` alone, so each of the three writable statuses has exactly one capability, one scope and one audit action. **8.1's stated premise was already closed by Phase 6.5** — verified before building — so it was executed for the reason that had NOT closed: a retirement was recorded as intent `lock` and rendered "Locked card" on the Audit page. **Step-up deliberately omitted** (Miki, 2026-08-16); typed last-four is the gate. ⚠ **`card_unlock` is STILL UNPROVEN** — its run voided correctly because ••••7671 was ACTIVE — and because all three runs started from ACTIVE, **H14's fix was never exercised**: every revert routed to `card_unlock`, which carries no status. See docs/22 H16 for the run that closes both. Mutation check ✅ 18/18 on the merge commit, CI ✅, Verify deployment ✅)* | `claude/fuelguard-phase-8-deactivate-jhpsmd` |
...
>
> ⚠ **`EFS_CARD_CONTROL_PROBE_ENABLED` is currently TRUE on `@fleetguard/api`.** Standing rule 15:
> unset it and redeploy once the proving is done. It is a real-card write path on a live service.
>
> **Still unreachable:** the HOLD → `card_lock` `{status:"Hold"}` revert, the only one that carries a
> status. It needs a card resting at Hold, which QA does not have (§0.6, finding F). Until that
> fixture exists `canonicalEfsStatus` is proved by unit test and mutation, not by a live run.
...
|---|---|---|---|
| 2026-08-16 (Phase 8, live) | 8 | **Step 8.2 PARTIAL — 2 of 3 proven on QA** · proofs `7cedf66c`, `88576345`, `6f28e05d` | **`card_deactivate` was proven on its first ever live run** — a capability built entirely offline, with a new ledger intent and a new approver scope, composed and landed against real EFS with no correction (717 ms, wrote the account's own `INACTIVE`, `nested:header`). `card_lock` re-proven at 514 ms. **But the runs did not test the thing this phase fixed.** All three used ••••7671, resting at ACTIVE, so every revert resolved to `card_unlock` — whose body carries NO status field — and H14's canonicalisation branch was never reached. `oeg5RevertLanded: true` twice is evidence the revert ROUTING works and is not evidence the CASING fix works; two claims, one proved. **`card_unlock` is still unproven**: its own run voided, correctly, because the card was already Active. The run that closes both is `prove card_unlock` against ••••7675 (INACTIVE) — it reverts through the INACTIVE → `card_deactivate` branch, which is exactly what H14 repaired. The HOLD → `card_lock {status:"Hold"}` revert stays unreachable until QA has a card at Hold (finding F). **Railway configured by Devin**: the CLI shows the project as `serene-elegance` with services `@fleetguard/api` and `@fleetguard/web` — `fleetguardapi` is a DOMAIN prefix and `--service fleetguardapi` fails, which cost one confused command. `EFS_CARD_SYNC_MAX_DETAIL` and `EFS_CARD_CONTROL_PROBE_ENABLED` were both **unset**, not merely low — so finding I's feared `200` was the code default, and it is now explicit at **500**. Finding **H closed**: `EFS_ROUTES_ENABLED=false` on the web service, owed since Phase 5. ⚠ **The probe flag is still TRUE** and rule 15 is outstanding. **A step-up refusal from mismatched credentials** (operator error, no card touched) named the actual cause — *"the password must be the one for the account whose token you pasted"* — rather than a generic auth failure, so it is kept as evidence the refusal messaging works. |
