...
| 3 | Capability architecture | 🔶 *(3.1–3.5 merged. **3.6 is 2 of 4** on `delivery-p3-migrate-four`: `card_unlock` and `override_grant` done, `override_clear` and `prompts_set` outstanding — see the Step 3.6 notes for the open question on each. 3.5a withdrawn — it is Step 4.2)* | `delivery-p3-registry` |
