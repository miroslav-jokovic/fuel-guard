...
| 9 | Driver assignment & prompts | 🔶 **9.1 and 9.2 DONE, merged and DEPLOYED 2026-08-17** *(migration **0200** in the ledger at `381aec4`; Verify deployment green on both hosts. The editable prompt set is resolved from `getPromptTypes` ∩ the guide's Info IDs table − PPIN — **24 ids, identical on both orgs** — cached per org on `efs_card_control_settings`, and threaded to the write path through `PlanCtx`. QA's cache confirmed filled live: `cached: {ok:true} | types: 41`. The schema now takes all seven validation types, the accrual value, the length-gated bounds, and no 2-prompt cap. **Step 9.5 is RETIRED — measured at 82 lines against a 200 cap.** **9.3, 9.4, 9.6, 9.7 not started.** ⚠ Two blockers stand: `proofPrompts` still samples the hardcoded pair, so a `prove prompts_set` would cover 2 ids not 24 and must be fixed before 9.7; and 9.4's live check needs the `infoSource=POLICY` fixture finding F has wanted since Step 0.13)* | `delivery-p9-prompts` |
