...
| 9 | Driver assignment & prompts | 🔶 **9.1, 9.2, 9.4 and 9.6 DONE and DEPLOYED 2026-08-17. 9.3 wire half only. 9.7 NOT STARTED — it is a live run.** *(migration **0200** at `381aec4`. The editable set is resolved from `getPromptTypes` ∩ the guide's Info IDs table − PPIN, cached per org, threaded to the write path — **and since 2026-08-17 shipped to the BROWSER on `capabilities.editableInfoIds`**, which was 9.1's missing half: the API validated 24 ids while the drawer offered the hardcoded 2, and a second instance of the same bug was found in the drawer's `readOnlyPrompts`. **9.4** refuses a card-level prompt write on an `infoSource: POLICY` card — the failure a re-read CANNOT catch, since the card stores the records it will never consult. **9.6** made Remove a third explicit action and took removal OUT of the Edit form (one destructive write, two routes = audit P0-3), and the confirmation now names prompts rather than vendor codes. **9.5 RETIRED** — 82 lines against a 200 cap. ⚠ **Three blockers, none of them code.** 9.3's display half is blocked on §7 Q1: we may show `1–1800` but may not yet explain it, and `RawInfo` needs widening to carry `minimum`/`maximum`/`value`. 9.4's live check needs an `infoSource=POLICY` card and **neither account has one** — every card on both reads `BOTH`. 9.6's two live checks and all of 9.7 need a QA card and an operator at a terminal. **The two blockers this row claimed on 2026-08-17 are both GONE**: `proofPrompts` was fixed by PR #80 and reads `ctx.editableInfoIds`, and 9.5 was retired — the row went on asserting both for a day.)* | `delivery-p9-prompts` |
