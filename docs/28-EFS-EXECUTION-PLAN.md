...
| **10.3** | UI: product select from the account's limit IDs, amount with the **unit spelled out** (gallons for fuel and DEF, dollars otherwise via `formatLimit`), window hours, min hours. Confirmation names product, amount with unit, window, uses. ⚠ **BLOCKED on F3** — do not build a screen for a feature that may be inert. Also note the drawer's body already sends `limits: []` explicitly, so the wiring point is the picker only | *"the confirmation names the product, amount and unit"* · *"an amount above 9999 is rejected before submit"* · and the `limitId` picker must be fed from the ACCOUNT (`getProducts`), which is where an unknown id is refused — the schema checks shape only, deliberately (Step 9.2's call for `infoId`) |
| **10.4** | Prove on QA. ⚠ **TWO cards, in this order (F8)**: first the **restore check** on ••••7672, the only QA card with card-level limits — grant a product override, clear it, re-read, and record whether DEF 250 / RFR 75 / ULSD 500 came back; then the **sequence-position** run on a reserved empty-`<limits>` card, which is consumed by it (docs/24 §3.3). ⚠ Answer **F3 first** or the restore check may be reading a card whose limits were never enforced | Full OEG. The restore observation is a **precondition for 10.5**, not a closing check. Offline equivalents of both runs already pass — see 10.1's Verify column |
| **10.5** | Promote. ⚠ Blocked on F3 and on 10.4's restore answer | Outcome recorded |
...
- [ ] ⚠ **F3 answered**: does a card-level `<limits>` override apply on a card whose `limitSource` is `POLICY`? *(docs/37 §7 Q7)*
