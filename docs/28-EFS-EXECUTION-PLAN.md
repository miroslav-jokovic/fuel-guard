...
**⚠️ REMAINING, and it needs a live run:** the counts above are simulated from the mirror, not observed after a sweep. `supabase db push` for 0195, then the next card sync populates `fuel_card_link`. **The number to check is how many rows carry `method: "pan_exact"`** — if that is zero, the two systems store card numbers differently and the PAN tiers are inert, which is the one hypothesis this step could not test offline.
