...
| 3 | Capability architecture | 🔶 *(3.1–3.6 done; 3.6 on `delivery-p3-migrate-four`, unmerged. **All five capabilities run from the registry** and `control.ts` is 483 → 135 lines. **Step 3.7 is next.** 3.5a withdrawn — it is Step 4.2)* | `delivery-p3-registry` |
