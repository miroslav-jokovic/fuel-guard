...
| 3 | Capability architecture | 🔶 *(3.1–3.7 done; 3.6 in PR #29, 3.7 on `delivery-p3-delete-old-path`. **`CardMutationIntentSpec` and `control.ts` are deleted** and all four Phase-3 waivers are gone. **Steps 3.8–3.10 remain**, plus the live QA owed on 3.5/3.6. 3.5a withdrawn — it is Step 4.2)* | `delivery-p3-registry` |
