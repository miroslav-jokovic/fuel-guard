...
| 5 | Operational readiness | 🔶 **CODE COMPLETE — ten of eleven steps; the exit gate needs live QA** *(**5.1 · 5.2 · 5.3 · 5.4 · 5.5 · 5.6 · 5.7 · 5.9 · 5.10 · 5.11 done**, all 18 `ci.yml` gates green, matrix 381/38/61/25, mutation 7/7, apps/api 1287 tests. Migrations **0196** and **0197** written and NOT YET APPLIED — `supabase db push` is Miki's. **5.8 is blocked by the plan itself**: it says decide with Step 7.1's real output and 7.1 is unrun. **Three exit-gate items need a live QA org** and were not faked — trigger each 5.1 signal, walk containment with timings, and confirm `mutation-check` no longer cancels itself (only observable on GitHub after merge). Recipes for all three are in `docs/29-EFS-INCIDENT-RUNBOOK.md` §6. **Also owed on Railway:** `EFS_ROUTES_ENABLED=false` on `fleetguardweb` and a `WEB_URL` repository variable, without which 5.10 is only half in effect)* | `claude/phase-5-operational-readiness-ebghxw` |
...
- [ ] **`mutation-check` no longer cancels its own runs on `main`.** `cancel-in-progress: false` is
      committed and the YAML parses; the step's Verify — push twice to `main` within a minute and
      confirm both runs complete — is only observable on GitHub after this merges.
