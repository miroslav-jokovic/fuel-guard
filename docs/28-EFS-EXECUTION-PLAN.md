...
| 6 | Drawer shell | ✅ **CODE COMPLETE 2026-08-16 — one manual check outstanding** *(6.1–6.4 built; `CardControlDrawer.vue` and its three panels deleted. **Three blockers found by survey, all fixed rather than worked around:** invariant 5's `preflightStepUp` is API-side, so the three step-up predicates moved to `packages/shared/src/efs/stepUp.ts` where the gate and the drawer read ONE definition, bound by a pin each side derives independently; the five `capabilities/*.view.ts` modules were dead code and are now what the drawer renders from, with `cardControlModel.ts`'s duplicate confirmations deleted (490 → 379 lines); and **`docs/34` missed a third** — per-capability promotion state and the environment never reached the browser at all, so invariants 6 and 7 needed `capabilityStates` + `environment` on `capabilities`. Matrix **381/38/61/25/17** re-run and confirmed, mutation **18/18** after repointing `efs-fraud-stepup-exact-match` at the rule's new home. **Outstanding: the Step 6.3 manual check** — list → kebab → Lock card… → confirm, in a browser)* | `claude/fuel-guard-phase-6-drawer-77e42o` |
...
| 8 | Card status *(first production promotion)* | 🔶 **8.1 DONE 2026-08-16 — 8.2–8.4 need Miki's Mac** *(migration **0199**. `card_deactivate` built as contract + behaviour + view; `card_lock` narrowed to `Hold` alone, so each of the three writable statuses now has exactly one capability, one scope and one audit action. **8.1's stated premise was already closed by Phase 6.5** — a held card could reach Inactive in one write since PR #67, verified before building — so the step was executed for the reason that had NOT closed: a retirement was recorded as intent `lock` and rendered "Locked card" on the Audit page, which is audit P0-3's mislabelling half. **Step-up deliberately omitted** (Miki, 2026-08-16); the typed last-four confirmation is the gate. **A live defect was found and fixed first**: every status proof revert fed the account's raw casing into a case-sensitive enum, so Step 8.2's first run would have left a QA card dirty. Gates green, matrices 381/38/61/25/17, mutation 18/18)* | `claude/fuelguard-phase-8-deactivate-jhpsmd` |
| 9 | Driver assignment & prompts | ✅ **CODE-COMPLETE 2026-08-17.** 9.1 (both halves), 9.2, 9.3, 9.4, 9.6, 9.7 done and deployed; 9.5 retired. *(migration **0200** at `381aec4`. 9.1's client half shipped the account's set to the browser on `capabilities.editableInfoIds` — the API had validated 24 ids while the drawer offered the hardcoded 2, and the same bug was found a second time in `readOnlyPrompts`. 9.3's display reports the odometer window as configuration — "minimum 1, maximum 1,800" — and a test asserts it does NOT explain the direction, which §7 Q1 has not settled. 9.4 refuses a prompt write on a `POLICY`-source card: the failure a re-read cannot catch, since the card stores records it never consults. 9.6 made Remove a third action and took removal out of the Edit form. **9.7 proven and promoted** — QA card ••••7671, proof `14723221-b664-4a22-8197-99ed9b930c68`, OEG 1/3/4/5 true, `docs/22` H6.)* ⚠ **Two live checks outstanding, and one of them cannot be run by anyone today:** 9.6's add-prompt pair needs Miki at a terminal; 9.4's needs an `infoSource=POLICY` card that neither org has. **Step 9.8 is NEW and open** | `delivery-p9-prompts` |
| 10 | Override with amount | 🔶 **10.1 done, 10.2 dropped as already-built, 10.3–10.5 blocked** *(2026-08-17. Verifying before implementing found eight things — see §10's findings table. **Two would have shipped green**: 10.1's specified `removals: []` is refused by `assertCollectionsPreserved` on every card that HAS limits, and works only on the empty-`<limits>` card 10.4 proves on; and the step-up message would have told an operator granting one use that they had asked for more than three. **F3 was then downgraded the same day** by reading WEX's own eManager portal guides, which document the override flow as never touching `limitSource` (F12). **The blocker is now F9** — all three portal guides say *"when a card is in override no changes can be made to the card"*, which is a `card_lock` safety question before it is a Phase 10 one, and is absent from the SOAP guide entirely — docs/37 §7 Q6. 10.2 was already done in Step 6.2)* | `claude/efs-phase-10-write-path` |
...
### ✅ Step 4.7 — `apply_latency_ms` does not measure what its column says — **DONE 2026-08-15 night**
...
**✅ DONE 2026-08-15 night — `orchestrator/dispatch.ts`, `orchestrator/types.ts`, `harness/prove.ts`, 4 tests.**

The measurement moved **into `verifyStep`**, which is the only place that knows which re-read saw the change — `prove.ts` was outside the orchestrator and could not have known. Its clock starts the instant `dispatchStep` returns, which is the same instant `writeProbeRealChange.ts` starts `verifyStarted`, so the two now report the same quantity by construction rather than by coincidence. **No migration — the column's documented meaning was right all along.**

Three rules, each stated because each could reasonably have gone the other way:

- **The retry pause is INCLUDED** when the second look is the one that lands. The column says *"the first re-read that saw the change"*, and a vendor whose edit only becomes visible after a wait genuinely took longer. Subtracting it would erase the only thing separating that case from an immediate landing.
- **On a sequence it is the LAST landed step's latency, not the sum.** The question is "how long does this vendor take to apply an edit"; a sum answers "how long did this capability take", which the ledger's timestamps already give you, and would make a two-step capability look like a slower vendor.
- **Null unless a step landed.** Zero would be a measurement; null is the absence of one.

> ### ⚠️ The first draft of the tests was worthless, and it passed
>
> Four cases, all green against **both** the old and the new measurement. A scripted vendor answers
> instantly, so with every call at ~0 ms the two differ only by the retry pause — which both include.
> The tests asserted the bug and the fix equally well.
>
> The fix was an artificial **400 ms delay on the write**: inside the old window, outside the new one,
> so the old measurement cannot come in under it and the new one cannot come in over it. Restoring the
> whole-call timing now turns all four red; before, it turned none.
>
> The same trap caught the comparison test. *"delayed − immediate ≥ the retry interval"* holds on the
> broken code too, because the pause is in one and not the other either way. What the old measurement
> **cannot** do is put the immediate case UNDER the retry interval — that clause is what makes the
> comparison mean anything.
>
> **The general rule, and it is the third time this build has paid for it:** a test written against a
> stub that is instant in every dimension cannot measure a defect that lives in one of those
> dimensions. Ask what the broken code would score before believing a green run.
...
### ✅ Exit Gate — Phase 4 — **CLOSED 2026-08-15 night, every item observed live**
- [x] **`pnpm efs:echo-scan` green across the production account — RE-RUN 2026-08-15 night: 197 scanned, 197 passed, 0 failed** (`docs/22` H11). One uninterrupted invocation, six batches tiling 0–196 with no gap and no overlap, so the claim needs no arithmetic across runs. The line's original "199" is stale — the account returns **197**, and the two extra mirror rows are cards WEX de-listed and `absent_since` correctly tombstoned. The CLI now pages to the end itself and names the org it scanned; batch size turns out to be bound by the route's 60-second wall clock (28–34 per call), not by `--limit`
...
- [x] **Promotion refuses on shape mismatch and on `unobserved` — BOTH paths now run live.** The RULE is proven by 19 unit tests over `decidePromotion`. **ALLOW:** `card_lock` promoted on QA 2026-08-15 citing proof `40b88b75`, both residual risks recorded. **REFUSE, 2026-08-15 night, production (`docs/22` H9):** HTTP 409, and **verified inert in the database** — zero promotion rows, zero settings rows, zero audit rows, all five promotions still QA-only. **It found a defect:** the gate named ONE blocker where production has THREE, because `decidePromotion` returned early on a missing proof and never reached the document-shape or vocabulary checks — so the route's promise that "every reason is returned" was false on the most common starting state, a company never promoted anything. Fixed in the same session; the proof-DEPENDENT checks are skipped when there is no proof (they would be noise) and every ORG-level check still runs
- [x] **Suspension refuses a live operation immediately — MEASURED 2026-08-15 night on QA: 481 ms, the very next call.** `card_lock` suspended, a real `POST /:id/lock` attempted immediately in the same process, refused `403 card_control_suspended`, then re-enabled. Verified in the database rather than from the response: QA `card_lock` back to `enabled`, card ••••7671 still `ACTIVE`, and **no mutation row was created at all** — the gate refuses before the ledger opens, so a suspended capability leaves no trace on the card path. 481 ms is an upper bound dominated by one HTTPS round trip, which is what "no cache" predicts and now also what it measures.

  > **The drill's first run suspended the WRONG COMPANY** — `docs/22` H10. Its safety was designed entirely around the card (a stale `expectedVersion`, so a failed suspension is still refused by optimistic concurrency) and not at all around the org, so a production token silently pointed it at production. It left one `card_lock = suspended` row there, which is being kept deliberately: it refuses HARDER than the `not_promoted` state it replaced, its reason explains itself, and Phase 8 clears it through the app. A second defect stopped the write before the gate — no `Idempotency-Key`, which the routes validate as a uuid first — so that run measured nothing. Both fixed in the drill; **the product behaved correctly throughout.**
- [x] **Standing gates green** — full suite, matrix 381/38/61/25, 7/7 registry mutations, all sixteen `ci.yml` gates
- [x] **Step 4.7 DONE** — the measurement moved into `verifyStep`, which is the only place that knows which re-read saw the change. The proof harness and the entitlement probe now report the same physical quantity. No migration: the column's documented meaning was right all along, the code was measuring something else and calling it that
...
- [x] **Signals fire when triggered — PARTIALLY OBSERVED, and closed on that basis.**
      ...
      trigger recipe per signal is `docs/29-EFS-INCIDENT-RUNBOOK.md` §6.2.
      The sixth signal, the weekly digest of unknown vendor elements, is **deliberately not built**:
      there is no "elements the product understands" set in the codebase to subtract from, so
      "unknown" would have to be invented, and a digest full of false unknowns trains people to
...
- [x] **`approved_by` populated; promotion authority defined.** Written through the
      ...
      mutation that leaves `pending` without an approver impossible, following 0142's precedent.
      Authority is recorded in §14, and its production half — the promoter may not be the person who
      ran the cited proof — is enforced in `decidePromotion` and tested as data.
- [x] **The `fuelCardsControl.test.ts` intermittent is reproduced and fixed.** It was never that file:
      it is keep-alive sockets holding `server.close()` open until the hook times out. All twelve
      suites now tear down through `closeTestServer`, and `testServerTeardown.test.ts` fails the
      build if one stops. Soak result is in §14.
- [ ] **`mutation-check` no longer cancels its own runs on `main`.** `cancel-in-progress: false` is
      merged, and the run on `7c6cfac` completed rather than being cancelled — but that is ONE push,
      and one push could never have been cancelled anyway. The step's Verify is two pushes to `main`
      inside a minute with both runs completing, and that has not happened yet. Not ticked: the
      evidence so far is consistent with the fix and equally consistent with no fix at all.
...
> **Done 2026-08-16 as ONE commit, not six.** The six operations are rows in `cardOperations.ts`, not six code paths: `CardOperationDrawer.vue` implements the snapshot, the dirty guard and the `sent` retry lock once, and reads the table for what differs. Six commits would have split one table six ways. Standing rule 8 is about keeping a step reviewable, and a per-operation diff of a data table is less reviewable than the table.
>
> **`reason` is per capability, not always-optional.** Decision B1 made it optional API-wide; `ReasonRule` then made it per-capability, and `override_grant` and `prompts_set` both declare `required`. The drawer reads the contract rather than assuming either way — so the note above is right that `docs/22` is stale, and right for a narrower reason than it states.
...
### ✅ Exit Gate — Phase 6 — **CODE COMPLETE 2026-08-16; one item needs a browser**
- [x] One test per invariant, all green — `CardOperationDrawer.test.ts`, 29 cases, each named for the defect it prevents
- [x] Every operation shows a diff before confirming — rendered from each view's `diff`, asserted per capability in `capabilities/views.test.ts`
- [ ] `reason` reaches the ledger; `Why` column populates — **the client half is done and the server half was already right**; every operation now sends a `reason` and the two capabilities whose contract says `required` refuse without one. What is NOT done is watching a row land with a populated `Why`, which needs a live write. Fold into Phase 8's first production promotion rather than spending a QA write on it alone
- [x] Old drawer deleted — `CardControlDrawer.vue` plus `CardStatusPanel`, `CardOverridePanel`, `CardPromptsPanel`; assertions re-homed, 49 `expect(` removed against 89 added
- [x] Standing gates green — all sixteen, plus `mutation-check` 18/18 and the `efs-card-control-triggers` matrix at 17

**Three blockers the plan did not anticipate**, each fixed in its own commit rather than worked around:

1. **Invariant 5 was not implementable as written.** `preflightStepUp` is a field on `CapabilityBehaviour`, invoked during `accept()` — the POST that performs the write — and `lint:boundaries` keeps the web out of `apps/api`. The three predicates moved to `packages/shared/src/efs/stepUp.ts`; the API's gates and the drawer's warning now read the same functions. Their different TIMINGS are unchanged, which matters: only `override_grant` decides from the body alone, so the drawer's warning for `card_unlock` and `prompts_set` is a courtesy against the mirror and **the `step_up_required` fallback stays**.
2. **The view registry was dead code**, which is what Phase 6 existed to consume. `CardOperationDrawer.vue` renders from `CARD_CAPABILITY_VIEWS`, and `cardControlModel.ts`'s five duplicate confirmations are deleted — 490 → 379 lines, so the file-size pressure `docs/34` warned about resolved itself.
3. **`docs/34` missed one.** Invariants 6 and 7 want promotion state and the environment in the header, and *neither ever reached the browser*: the read path deliberately calls `loadCardControlAccess` without a `capabilityKey`, so nothing about promotion was on the wire. Visible on QA, where `override_grant` is correctly unpromoted — the drawer showed the button and the API answered 403 after the operator pressed it. Fixed by adding `capabilityStates` and `environment` to `capabilities`.

**Two corrections carried in from Step 6.2**, both in `cardOperations.ts` with their reasons: Deactivate is available on Hold as well as Active, and Remove exception appears on override *residue* (a scope field armed with zero uses left), not only on an active exception.

**Outstanding — the Step 6.3 manual verify, now narrowed to the part a test cannot reach.** *"list → kebab → Lock card… → confirm, in two interactions"*. The kebab NAVIGATES to `/fuel-cards/:id?action=lock` rather than opening a drawer on the list, because `EfsCardRow` carries no `card_version` and no prompt records — the version is the concurrency token every write must carry, and `prompts_set` is a full `replaceAll`, so an editor opened with no records is a write that deletes the driver assignment (guide p137).

The LINK half is automated: `operationLink` is the only place `?action=` is spelled, `operationFromQuery` is the only place it is read, and `cardOperations.test.ts` puts the builder's output through a URL parser into the parser for all six operations. What is left needs a browser and a populated card list, and this container has neither — the network policy answers 403 to `*.supabase.co`, so the page renders with no rows and therefore no kebab. **What a human still has to confirm:** that the menu appears in the row's action column, that the card page opens with the drawer already on that operation, and that Confirm is reachable without a third click.
...
**DONE 2026-08-15 night — migration 0195, `efsCardLinking.ts` (new), `cardIdentityLabel.ts` (new), 21 tests.**

**The root cause was written down in this repository and never joined up.** `linkFuelCards` decided identity with `cardRefsMatch`, whose own doc comment says it is *"a COMPATIBILITY test, not an identity test"* and that anything deciding two rows are the same card must not use it. `cardRefsDefinitelyMatch` already existed for exactly this and was never called. The warning and the violation sat two files apart for months.

**Three tiers, each requiring uniqueness:** `pan_exact` (the sealed EFS number equals `fuel_cards.card_ref` — all 173 production refs are distinct 19-digit PANs, so a hit is an identity) → `pan_suffix` (one full number is the trailing suffix of the other, both ≥ 8 digits, because the two systems are not guaranteed to store the same LENGTH) → `last4_unit` (`unit_prompt` resolved against `vehicles.unit_number`; two cards sharing a last-4 on the SAME unit are still refused).

**Measured before building.** The unit tier alone resolves **100 of the 143 unlinked live cards, with zero left ambiguous**. 5 have no `fuel_cards` counterpart at all and can never link.

**The plan's "(last-4 + driver)" tier was NOT built.** `fuel_cards.driver_id` is null on all 173 production rows, so it would have been dead code that reads like coverage.

**It fails safe on the one thing offline work cannot check.** Whether EFS's card number equals the imported `card_ref` is unprovable from this side — both are sealed. Tier 1 needs exact equality and tier 2 an 8+ digit suffix, so if the formats disagree those tiers match **nothing** and it falls through to the unit tier. Fewer links, never a wrong one. `fuel_card_link.method` records which tier fired, so **one live sweep answers what no amount of reasoning here can** — the same lesson as Phase 3.

**Four ways to fail, because each sends somebody somewhere else:** `ambiguous` (add a unit prompt) · `unconfirmed` (one likely partner, but a last-4 is not proof — a person confirms it, the sweep will not) · `no_candidate` (import the missing row) · `unidentifiable` (the mirror row carries nothing to match on). Collapsing `unconfirmed` into `no_candidate` would tell an operator nothing is there when something probably is.

**Migration 0195 adds `fuel_card_link` jsonb, deliberately NOT `sync_error`** — that column already carries a refresh outcome AND a linking outcome, which is Step 7.5's complaint. The linker now writes neither into it, so **7.5's linking half is resolved here** and only the refresh half remains.

**The reading half.** `••••7552` names a GROUP on this fleet, and `ActiveOverridesPanel` printed `driverName ?? unitPrompt` — which hides whichever field it does not pick, and that is often the only discriminator. `cardIdentityLabel` shows unit AND driver, and says "no unit or driver" out loud for the **37 live cards with neither**, rather than printing a masked ref that reads like an identity.

**Guard verified by breaking it:** reverting tier 1 to a last-4 comparison turns exactly one test red — the one named *"THE CASE THIS STEP EXISTS FOR"*. `efsCardLinking` was split out of `efsCardMirror` at the 500-line budget; split, not waived.

**✅ PROVEN LIVE 2026-08-15 19:31–19:37Z — `docs/22` H8.** 0195 pushed, manual sweep run against the production org, 197 cards read, 0 failed:

| | before | after |
|---|---|---|
| linked (of 197 live) | 54 | **157** |
| by `pan_exact` | — | **103** |
| by `pan_suffix` / `last4_unit` | — | 0 |
| `ambiguous`, naming their candidates | — | 35 of 35 |
| `no_candidate` | — | 5 |
| `sync_error = ambiguous_fuel_card_link` | 139 | **0** |

**The hypothesis resolved in the best direction.** `pan_exact` fired 103 times, so EFS and the fuel import DO store the same 19-digit number and the PAN tiers are not inert. The previous sweep, on the old last-4 linker, linked **2**. Fuel attribution now runs on **80% of the live fleet, up from 27%**.

**The prediction was right about the count and wrong about the mechanism.** Simulating before the build said the unit tier would resolve ~100 of 143; it resolved 0 and the PAN tier resolved 103. That is exactly why `method` is recorded per row rather than inferred from a total — a matching headline number would have hidden a completely different story underneath.

**Keep the lower tiers.** `last4_unit` fired zero times because `pan_exact` reached everything it could, not because it is redundant: it is what catches a card whose PAN never made it into an import.
...
| Step | Change | Verify |
|---|---|---|
| **8.1** ✅ | ~~Add **Deactivate**, available on Active **and** Hold — today retiring a held card requires unlocking first, momentarily re-enabling fuel purchases.~~ **That premise was closed by Phase 6.5 (PR #67) before this step ran** — verified, and 8.1's Verify already passed. Executed instead for the audit trail: `card_lock` wrote both statuses, so a retirement was recorded as intent `lock` / action `card.locked` and rendered "Locked card". New capability `card_deactivate` (contract + behaviour + view) with its own intent, scope and audit action; `card_lock` narrowed to `Hold`; migration **0199**. Typed last-four confirmation. **Step-up deliberately NOT added** — see the contract header | *"a held card can be deactivated without first being unlocked"* — asserted on the WIRE (one `setCardv2`, `INACTIVE`, no `ACTIVE` in any request) |
...
### ✅ Exit Gate — Phase 8
- [ ] Three capabilities proven on QA and recorded in `docs/22` — **8.2, needs Miki's Mac**
- [ ] Promotion attempted for production; outcome recorded — **8.4, needs Miki's Mac**
- [ ] QA cards restored; standing gates green — gates ✅ green on `claude/fuelguard-phase-8-deactivate-jhpsmd`; QA cards untouched (no live run from a container)

> **Run 8.2 before anything else, and run `card_unlock` first.** It has never been proved. Its proof
> needs a NON-Active card and QA has exactly two (both INACTIVE), so it is the run that would have hit
> the revert defect this session fixed — and it is the cheapest confirmation that the fix holds against
> a real card rather than against a schema.
>
> **`card_deactivate` cannot be proved on QA without a fixture.** Its precondition needs a card that is
> not already Inactive and that some capability can put back; QA's two non-Active cards are both
> INACTIVE, so the run voids. **Prove it on the ••••7682 Active status card**, whose revert routes
> through `card_unlock` — or create the Hold fixture §14 has wanted since Step 0.13, which would also
> exercise the case the step was written for.
...
- [x] Odometer following settable with its accrual value and displayed correctly — **both halves, within what may honestly be claimed.** Wire in PR #81; display 2026-08-17. `RawInfo` widened, and the prompt renders "Driver enters the odometer — minimum 1, maximum 1,800". ⚠ The step's own sentence ("the pump rejects a reading more than N miles from the last one") is NOT used and must not be added until `docs/37` §7 Q1 is answered — it asserts a direction the guide never describes. A test asserts its absence
...
**Phase 9 is CODE-COMPLETE, and two live checks remain that this container cannot run.**

Every gate line above holds. What is outstanding is evidence, not implementation:

| Outstanding | Who can close it | Blocked by |
|---|---|---|
| 9.6 — *"can add a prompt the card does not have"* / *"cannot add one already on the card"* | Miki, on the reserved empty-`<infos>` QA card | needs a terminal, a token and a card number |
| 9.4 — a card-level prompt write on a `POLICY`-source card is refused | **nobody, today** | every card on BOTH orgs is `infoSource: BOTH`. Proven offline against `getCardV2.empty.xml` instead |
| §7 Q1 — the accrual window's direction | WEX | blocks only the explanatory SENTENCE, not the feature |

**9.8 is new and open** (see the row above): `emittableValues` declares less than the capability
emits, which the 9.7 proof demonstrated by sending `NAME`.
...
### ⚠ Verified 2026-08-17, BEFORE implementing — eight findings, two of which would have shipped green

The method again. Every claim below was checked against the code, the checked-in WSDL and the guide's
own field tables rather than against p194's recipe alone.

| # | Found by verifying, not assuming | Would have shipped as |
|---|---|---|
| **F1** | **10.1 specified `removals: []`.** `assertCollectionsPreserved` (efsCardCollections.ts) refuses a request that omits any `<limits>` record the card arrived with unless the edit NAMES it, and p194's recipe drops them all on purpose | An override that works **only** on a card whose `<limits>` is already empty — i.e. passes 10.4, which proves on exactly that card, and is refused `echo_unfaithful` on the first real card. `efsCardXml.test.ts:376` already had this right with `removals: ["CADV"]`, so the plan contradicted a pinned test |
| **F2** | A **second, independent** reason for the same fix: a four-field override record on a card whose limit carries `autoRollMap`/`autoRollMax` trips the guard's FIELD-drop branch, not its record-drop branch (`getCardV2.autoRoll.xml` is that shape) | "Reuse the existing record and change the amount" adopted as a workaround, and refused for a reason nobody had connected to the first one |
| **F3** | ⚠ **`limitSource`.** Guide, limits object: *"Only limits assigned at the CARD level are returned, POLICY limits are not returned even if the card source is set to `BOTH`."* docs/22 H7 records `limitSource` as `POLICY` on **all 35** QA cards, and p194 never mentions the field | The whole phase inert: a card-level limits override accepted-and-ignored (audit W3), the operator told an exception was granted while the pump enforced the policy's 250 gallons. **Now docs/37 §7 Q7 — gates 10.3 and 10.5** |
| **F4** | The override **deletes** the card's product limits and nothing in the guide promises EFS restores them. 10.4 tested that; nothing recovered from a "no" | A vendor answer of "no" leaving a card permanently altered with no record of what it used to hold. `auditMeta.limitsBefore` now carries the exact records |
| **F5** | **10.2 was already done, in Step 6.2.** `cardOperations.ts` already renders Remove exception on `uses > 0` **or** a scope field armed, with its test at `CardOperationDrawer.test.ts:767` | A step re-built for reasons that stopped existing two phases ago — the same shape as Phase 9's 9.7 |
| **F6** | Two of 10.1's four Verify bullets **already exist**: `efsCardXml.test.ts:376` (the p194 recipe) and `:824` (a new limits collection landing before `locationGroups`) | The write path re-derived at the serializer level, where it was already pinned |
| **F7** | `OVERRIDE_GRANT_STEP_UP` is ONE string read by both the gate and the drawer. "Step up whenever limits are present" reusing it tells an operator granting ONE use *"confirm your password to grant more than 3 uses"* | A step-up prompt naming a threshold the operator is nowhere near, sending them to turn the stepper down |
| **F8** | **10.4 needs TWO cards, and QA holds one of them.** docs/24 §3.3: *"S4 and S5 must not share cards. S5 rewrites limits, so S4's headline observation — clearing restores the original limits — stops testing what it was."* H7's actual holding: empty-`<limits>` cards are plentiful; cards WITH card-level limits number **one** (••••7672: DEF 250, RFR 75, ULSD 500) | The restore check run on the empty card, where it reads empty before and empty after and passes while proving nothing |

Bounds **confirmed** against the guide's limits field table rather than p194's example: `limit`
numeric(4) *"a value from 0 to 9999"*, `hours` and `minHours` both `int (3)` → 0–999. The plan's
numbers were right; they had not been checked.

### ⚠ Second pass, same day — WEX's OWN PORTAL DOCUMENTATION, which none of the above had read

Three official eManager quick-reference guides were found and read (Nov 2017 Overrides, Nov 2017 Card
Management, Apr 2019 Administrator). **All three agree with each other**, the Overrides guide is now
checked in at `docs/efs/eManager-Overrides-2017-11.pdf`, and full citations plus the passages that
matter are in **docs/37 §8**. They answer two questions the 200-page SOAP guide does not address at
all, and they change three things about the steps below.

| # | Found in WEX's portal documentation | Consequence |
|---|---|---|
| **F9** | ⚠⚠ *"When a card is in override no changes can be made to the card (i.e. status, add cash, etc.), therefore, it is recommended that (1) one override/swipe be selected."* — in all three guides, under BOTH override flows. **Absent from the SOAP guide entirely.** | **The biggest find of the session, and it is not Phase 10's.** If true of the API and not just the portal's screens, then on a card with an armed override `card_lock` may not apply — the 2am stolen-card action this codebase keeps free of all friction. `prompts_set`, `card_unlock`, `card_deactivate` and the mileage override are in the same position and **none of them checks `overrideUses`**. ⚠ And our LIVE override clear is itself a `setCardv2` on a card that is by definition in override — which would explain why WEX built `deleteOverride` (p27) as a dedicated payload-free operation at all. **Now docs/37 §7 Q6, with a cheap QA probe. It should be answered before Phase 10 promotes** |
| **F10** | *"Override limit does not 'add' to the existing limit; it is REPLACING the limit as a daily total (i.e. if a card has a 100 gallon limit of diesel and the card needs an additional 50 gallons, the override would need to be in place for 150 gallons)."* | Confirms §1.2's premise from the vendor's own side — F4's `limitsBefore` is right to exist. **And it is a hard requirement on 10.3: the operator enters the TOTAL, not the increment.** A field labelled "additional gallons" grants 50 where the card already allowed 100, so the driver is declined for asking for more. Silent, and in the expensive direction |
| **F11** | *"When overriding fuel, add product DSL for the desired gallons … and also add product ULSD for the desired gallons. This is done because different truck stops use different product codes for fuel."* | **p194's ULSD-only example is not a complete operational recipe.** A diesel override needs both codes or it declines at the half of the network that uses the other one. 10.3's picker must pair them rather than leaving an operator to know this |
| **F12** | Q5/F3 **downgraded from blocking to confirming.** The portal's procedure for a PERMANENT card limit starts *"Under 'Limit', select 'Both'"* — so card-level limits ARE gated on the source field. But the OVERRIDE flow has no such step, and its stated use case is *"a product's limit has been used"*, i.e. cards whose limit came from a policy | **`override_grant` should NOT write `limitSource`.** Doing so would invent a step the vendor's own flow lacks and would leave a card permanently consulting card-level limits after a temporary exception — F3's harm, caused by the fix. p194 is complete for the override path. **10.3 and 10.5 are unblocked** |
| **F13** | *"Once the card is swiped the number of overrides selected the override will automatically be removed."* | The vendor manages the override lifecycle server-side. The strongest available indication that restoring the prior limits is WEX's job — and still not a statement that WEX does it, so 10.4's observation stands |
| **F14** | The override screen calls `hours` *"the number of hours allowed between swipes … the default is 1"*; the limits screen calls it *"when to refresh the limit (i.e. 24 equals daily)"*; the SOAP field table matches the second and puts *between uses* in `minHours`. p194 nonetheless writes `hours: 1, minHours: 0` | One label, two meanings — the family docs/22 keeps recording. **Default `hours: 1, minHours: 0` exactly as p194 does (which is also the portal's default), and do not label the field with either meaning in 10.3 until confirmed** |

⚠ **Method note worth keeping.** These documents do not extract through `WebFetch` — its reader reports
"corrupted binary" on all three. They extract perfectly with `pdftotext`. Download, then extract
locally. Two questions that had been written down as *"only WEX can answer this"* were answerable from
WEX's published material the whole time, which is a reason to check docs/37 §8 before opening a ticket.

Also worth one line so nobody conflates them: the recipe **immediately below** p194's product-limit
one, *"Override Refreshing Limits"*, **is** a multi-call flow — it needs `setCardRefreshingLimits`
against a card number with `OVER` appended. That is not this phase.

| Step | Change | Verify |
|---|---|---|
| **10.1** | ✅ **DONE 2026-08-17** *(commit on `claude/efs-phase-10-write-path`)*. `grantOverrideSchema` gains `limits: z.array({limitId, limit: 0..EFS_LIMIT_MAX, hours/minHours: 0..EFS_LIMIT_HOURS_MAX}).max(EFS_OVERRIDE_MAX_LIMITS).default([])`; a duplicate `limitId` and a location-scoped product override are both refused at the schema. `overrideGrantEdits` takes `limits` as a REQUIRED argument and builds `{op:"replaceAll", name:"limits", records, removals}` — **`removals` computed from the response DOM via the same `recordIdentity` the guard checks with (F1, F2)**. `overrideGrantStepUp` returns the sentence that actually fired, read by both the gate and the drawer (F7). `auditMeta.limitsBefore` records what the write deleted (F4) | ✅ *"sends the p194 limits array, in sequence position"* (exact bytes, one record, after `<infos>` before `<locationGroups>`) · *"names every pre-existing limit in removals"* **with the positive control that `removals: []` throws `drops <limits> record "CADV"`** · *"survives a card whose limit record carries the auto-roll fields"* · *"introduces the limits collection on a card that has none"* · schema: duplicate refused, location scope refused, each width bounded **with positive controls at each ceiling** · step-up: one use with limits gets the limits sentence and **does not contain the threshold** · 3 mutations, all caught (29/29) |
| ~~**10.2**~~ | ~~Fix override residue~~ — **DROPPED, already done in Step 6.2** (F5). `cardOperations.ts` `applies: usesLeft(card) > 0 \|\| card.overrideAllLocations === true \|\| card.locationOverrideId !== null` | Already pinned by `CardOperationDrawer.test.ts:767` |
| **10.3** | UI: product select from the account's limit IDs, amount with the **unit spelled out** (gallons for fuel and DEF, dollars otherwise via `formatLimit`). ✅ **UNBLOCKED by F12.** ⚠ Three portal-documented requirements it must meet: the amount is the **TOTAL, not the increment** (F10) — label and confirmation must say so, because the failure is a declined driver; a diesel override needs **both DSL and ULSD** (F11), so the picker pairs them rather than relying on operator knowledge; and `hours`/`minHours` default to **1 / 0** as p194 writes them, with **no meaning asserted in the label** until F14 is settled. Confirmation names product, amount with unit, uses. The drawer's body already sends `limits: []`, so the wiring point is the picker only | *"the confirmation names the product, amount and unit"* · *"an amount above 9999 is rejected before submit"* · and the `limitId` picker must be fed from the ACCOUNT (`getProducts`), which is where an unknown id is refused — the schema checks shape only, deliberately (Step 9.2's call for `infoId`) |
| **10.4** | Prove on QA. ⚠ **Answer F9 (docs/37 §7 Q6) FIRST** — if an armed override freezes the card, it changes what a proof run can even do, and it is a `card_lock` safety question that outranks this phase. Then: ⚠ **TWO cards, in this order (F8)**: first the **restore check** on ••••7672, the only QA card with card-level limits — grant a product override, clear it, re-read, and record whether DEF 250 / RFR 75 / ULSD 500 came back; then the **sequence-position** run on a reserved empty-`<limits>` card, which is consumed by it (docs/24 §3.3). ⚠ Answer **F3 first** or the restore check may be reading a card whose limits were never enforced | Full OEG. The restore observation is a **precondition for 10.5**, not a closing check. Offline equivalents of both runs already pass — see 10.1's Verify column |
| **10.5** | Promote. ~~⚠ Blocked on **F9** and on 10.4's restore answer.~~ ✅ **UNBLOCKED 2026-08-18** — F9 answered by H16/H17 (guards shipped), and 10.4 ANSWERED on production ••••6536: verdict `restored`, plus two wire discoveries (setCardv2 demands the six-field limit record; getCardv2 never echoes an override's limits) — docs/40 §1.3, transcript `docs/efs/limit-restore-production.json`. *(10.4 ran on production, not QA as this row planned: the fleet sweep proved every QA card is POLICY-source and cannot answer — docs/40 §1)* | Outcome recorded |

### ✅ Exit Gate — Phase 10
- [x] The write path builds p194's recipe correctly, including the removals the guard requires *(10.1, offline)*
- [x] Sequence ordering holds on a card with no pre-existing limits *(offline; the live run is 10.4's)*
...
- [x] ✅ **F9 ANSWERED 2026-08-17 — and the answer is yes, the freeze is real** *(docs/22 **H16**, docs/37 §7 Q6)*
  ...
  - ✅ **FOLLOW-UP ANSWERED (docs/22 H17): the combined write LANDS.** Status + `overrideClearEdits()` in ONE request applied both — `5b` landed where `4` did not, and `5c` read `uses: 0, status: HOLD`. So EFS judges the status against the request's own effect, not the card's arrival state, and **`card_lock`'s `clearException` is genuinely one press.** No copy change owed
...
| 2026-08-16 | **Promotion authority, defined (Step 5.3).** To promote a capability at all: role `admin`, org member, and a fresh session (`requireFreshAuth`) — all three already enforced. **On PRODUCTION, additionally: the promoter may NOT be the person who ran the proof they cite.** Enforced in `decidePromotion`, so it is tested as data rather than as a request. Outside production self-promotion is allowed and recorded as a residual risk on the promotion, because QA is where one person legitimately runs the proof and promotes it. A cited proof whose `run_by` is null fails closed and the refusal names the fix (run a fresh proof). Card MUTATIONS are not held to two people: plan and apply are one request today, so it would refuse every write; `approved_by` is now written truthfully as a self-approval instead, following migration 0142's precedent for loads | Claude (PM) |

### Sessions

| Date | Phase | Steps completed | Notes / surprises |
|---|---|---|---|
| 2026-08-17 (Phase 9) | 9 | **Steps 9.1 and 9.2 DONE and DEPLOYED** · migration **0200** · `resolveEditableInfoIds` · `cardControlPrompts.ts` · `cardVersion.ts` · `promptDrafts.ts` · `editableInfoIds.test.ts` · **9.5 RETIRED** | **`main` was not green when the session started, and the reason mattered.** `efs-card-control-triggers` failed 5 of 5 runs on a fast machine at `46ee6cf` unmodified — the matrix this plan pins at 17 was scoring 15 or 16. PGlite's clock is MILLISECOND resolution: `to_char(now(),'…US')` always renders `000`, and eight consecutive transactions produce about three distinct values, so two updates in one millisecond read back identical and `after !== before` fails against a trigger that fired correctly. It had been mistaken for a precision problem TWICE (String → microsecond text) when PGlite never had microseconds to give. Fixed with a planted `2000-01-01` sentinel — no clock reading compared to another — and it is strictly stronger: the Step 5.7 defect restored against the OLD test scored 4, 4 and 5 failures over three runs, a different set each time, including assertions that mutation cannot break and including genuine false passes. Against the sentinel it scores exactly 3, every run. **CI had been passing on luck**, because slower runners cross millisecond boundaries more often. PR #76. **Verified against the vendor's documents rather than this plan, on Miki's instruction.** The guide corrected me in BOTH directions: it confirmed 9.3's premise (`value` really is the ACCRUAL_CHECK accrual, p36/p135/p138) after production data made me doubt it, and it contradicted `docs/25` Q3, which records production odometer following as configured when the accrual is `0` — the guide's own not-configured sentinel — and `lengthCheck: false` makes the 1–1800 range inert. **Step 9.5 is retired**: it exists to decompose `promptsEdits` before a 200-line cap and it is **82 lines**; the two functions near budget are `createApp` (194) and `syncIdleFoundation` (180), neither Phase 9's. Folded into 9.2/9.3 as an acceptance condition. **9.1's resolved set is 24, identical on both orgs**, computed offline from the committed captures and confirmed live (`types: 41` on QA). The accounts return **15 codes on production and 16 on QA that the vendor documents nowhere** — the guide's Info IDs table has exactly 26 entries and `EFS_INFO_LABELS` transcribes it one-for-one. **PPIN denied** (Miki, 2026-08-16): editing it is issuing a credential, and `redactCardXml` masks digit runs of TEN OR MORE so a 4–6 digit PIN would ride in clear through the request, the stored XML and the audit diff. Consequence recorded: DYNAMIC reaches CNTN and DRID only. **A latent defect the widening exposed.** A submission for an id outside the editable set was never ignored — the pass-through loop skips entering it into `seen`, so the append loop pushed a SECOND record with the same `infoId`, audit P1-6b's duplicate shape. Unreachable only because the schema's static enum matched the hardcoded pair; resolving per account lets them disagree. `promptsEdits` now refuses. **Found by a positive control, not by review.** **Two of my own tests failed their mutations.** The casing assertion `["drid"," Unit "] → ["DRID","UNIT"]` passes with case-folding DELETED: nothing matches, the resolver falls back, and the fallback IS that pair — it passed against the bug and the fix alike. And `|| promptTypes.length === 0` was redundant code, not a weak test; the single fallback already covered it. **I ran 5 of the 19 gates `ci.yml` runs** and CI caught `lint:filesize` at 501 and 504 lines. Split, never waived — and the split then made a RUNTIME CYCLE (`cardControlPrompts` ⇄ `cardControlContract`) that blanked `cardVersionSchema` and reddened all ten prompt contract tests with "expected a Zod schema". `cardVersion.ts` is now a leaf that cannot participate in one. **Open, and it must be closed before 9.7:** `proofPrompts` still builds its sample from the hardcoded DRID/UNIT pair — `proof.sample`/`revert` take only a `Snapshot` — so a live `prove prompts_set` exercises 2 ids while the write path validates against 24. A green proof would cover less than it appears to, the same shape as the `oeg5RevertLanded` lesson. Mutation harness **18/18 → 21/21** (0196 both directions, 0200's `cardinality`/`array_length` — the one mutation whose real counterpart is in this repo's history). Matrices **381/38/61/25/24**. PRs #76, #77, #78; migration 0200 in the ledger at `381aec4`, Verify deployment green on both hosts. |
| 2026-08-16 (Phase 8) | 8 | **Step 8.1 DONE** · migration **0199** · `cardDeactivate.{contract,behaviour,view}.ts` · `statusRevert.ts` · `TypeToConfirm.vue` · `operationBlocker.ts` · `useOperationDispatch.ts` | **The step's stated premise was stale and the step was still worth doing — for a different reason.** 8.1 justifies itself with "today retiring a held card requires unlocking first"; Phase 6.5 (PR #67) closed that on 2026-08-16, the same day the handoff quoting it was written. Verified before building: `statusRows("HOLD")` already routed Inactive through `card_lock` in one write, so 8.1's Verify passed against `main`. What had **not** closed is the audit trail — `card_lock` wrote both statuses, so a retirement was intent `lock`, action `card.locked`, and "Locked card" on the Audit page: audit P0-3's mislabelling half, on the one screen an operator opens to find out what happened to a card. **A live defect was found first, and it blocked Step 8.2.** Both status proof reverts built `{status: snap.doc.card.status}` — the account's own `INACTIVE`/`HOLD` — and `lockCardSchema` is a case-SENSITIVE enum, so `prove.ts`'s `accept()` refused every one and threw: OEG-5 false, `cardStillChanged`, a QA card left at the proof's target status. It had not fired only because 33 of 35 QA cards are ACTIVE and revert through `card_unlock`, which carries no status; **`card_unlock` has never been proved, and 8.2 is its first run**. Fixed in one shared `statusRevert.ts` that sends the canonical spelling, plus a `statusIsRevertible` precondition so a Fraud/Deleted card is never written to with no way home. **Migration 0190 overruled my own scope decision** — I had settled on reusing `lock` (no new power, no migration) until its closing note turned up, reserving this exact call: "The first genuinely new scope arrives with `card_deactivate` in Phase 8.1 … in the same migration that adds the capability and its grant statement." 0199 does both and backfills `deactivate` onto every `lock` holder. **Validating 0199 against a scratch database found a defect 0173 has carried since it was written**: `array_length('{}',1)` is NULL, `NULL >= 1` is NULL, and a CHECK only rejects on FALSE — so "an approver row with no scopes is a person who looks authorised and is not" has never been enforced. Fixed with `cardinality` in the constraint 0199 was already rewriting; never exploitable, because the API refuses an empty scope list first. **Step-up omitted deliberately** (Miki's call on my pushback): deactivating STOPS fuel, the same direction as a lock, whose contract records that friction there "has a cost measured in stolen fuel" — the failure a retirement has is the wrong card, which the typed last-four addresses. **One claim was withdrawn rather than shipped**: a test asserting the frozen capability protects against a mid-flight draft change SURVIVED its mutation — `confirm()` enters `run()` synchronously, so no such window exists. The freeze stays as defence-in-depth and both the test and the comment now say exactly that. **Also fixed, found by the type checker**: `cardOperations.test.ts` built a draft naming `lockStatus`, renamed in Phase 6.5 and hidden by `as never`, so `capabilityFor` had been resolving from `undefined`. And `canDeactivate` had to reach `CardCapabilities` — `FuelCardDetailPage.vue` gates the whole action surface on the four `can*` booleans, so an approver narrowed to `['deactivate']` would have silently lost it. **Unreproduced:** one `rls` run reported 380/1 while `mutation-check` was saturating the CPU; nine consecutive clean runs of 381 afterwards, never reproduced, not investigated further. Mutation-verified: Inactive back through `card_lock` → 7 red · typed last-four removed → 2 · guide casing → 2 · `===` for `efsStatusEquals` → 1 · verbatim revert casing → 6 · `statusIsRevertible` unconditional → 3. |
...
| 2026-08-15 (night, later) | 2, 4 | **Step 2.7 DONE** · **Step 4.7 DONE** · `writeProbeJudge.ts` split out | **Two probes that lied about different things, both now honest.** **2.7:** `/write-check` upserted `write_entitlement` unconditionally, and `judge()` returns `unknown` for every read-only run BY CONSTRUCTION — `denied` needs a refusal at step 5 or 7, `confirmed` needs step 8, none of which run read-only. So the harmless diagnostic downgraded a `confirmed` org and stopped every card action in it. The fix is an OMISSION, not a branch: PostgREST's upsert sets only the columns it is given, so the key is simply left out and there is no window where the wrong value is stored. **Two things came with it.** The RESPONSE was telling the same lie — fixing only the write would have left the column saying `confirmed` while the response said `unknown`, which is worse, so it now carries `entitlement`, `judgedEntitlement` and `entitlementWritten` as three separate facts. And `probed_document_shape` had the IDENTICAL defect one column over: a probe failing at step 3 erased a shape an earlier probe established. Same sentence, same commit. **A read-only probe is now the cheap way to satisfy Step 2.6's binding** — it still records the identity hash, so a rotated credential can be rebound without touching a card. **4.7:** the measurement moved into `verifyStep`, the only place that knows which re-read saw the change; the proof harness and the entitlement probe now report the same physical quantity by construction. No migration — the column's meaning was right, the code was measuring something else and calling it that. **THE LESSON, and it nearly shipped:** the first draft of 4.7's tests passed against BOTH the old and the new measurement. A scripted vendor answers instantly, so with every call at ~0 ms the two measurements differ only by the retry pause, which both include — the tests asserted the bug and the fix equally well. An artificial 400 ms on the WRITE separates them: inside the old window, outside the new one. Restoring the broken timing now turns all four red; before, it turned none. **A stub that is instant in every dimension cannot measure a defect that lives in one of them — ask what the broken code would score before believing a green run.** **Also:** `judge()` split to `writeProbeJudge.ts` when 2.7's additions took the route past the 500-line budget — split, not waived, because a waiver pins the file at its current size and makes the next addition somebody else's problem. |
| 2026-08-15 (night, last) | 7 | **Step 7.7 DONE** · migration 0195 · `efsCardLinking.ts` · `cardIdentityLabel.ts` | **The root cause was written in this repository and nobody joined it up.** `linkFuelCards` decided identity with `cardRefsMatch`, whose own doc comment says it is "a COMPATIBILITY test, not an identity test" and that anything deciding two rows are the same card must not use it — and `cardRefsDefinitelyMatch` already existed, uncalled. Two files apart, for months. **The measurement is the finding:** 40 last-4 groups hold more than one live production card, covering 144 of 197; ••••7552 is five cards on five trucks. So the linker could only refuse or mis-attribute, and it refused — 143 unlinked, fuel attribution on a quarter of the fleet. **Three tiers, each requiring uniqueness:** exact full PAN, then full-PAN suffix (>= 8 digits, because the two systems need not store the same LENGTH), then last-4 + unit. **Measured before building** — the unit tier alone clears 100 of 143 with zero left ambiguous. **The plan's "(last-4 + driver)" tier was NOT built:** `fuel_cards.driver_id` is null on all 173 rows, so it would be dead code that reads like coverage. **It fails safe on the unprovable part.** Whether EFS's number equals the imported `card_ref` cannot be checked from this side — both are sealed — so the PAN tiers demand exact equality or an 8+ digit suffix and match NOTHING if the formats disagree, falling through to the unit. Fewer links, never a wrong one, and `method` records which tier fired so **one live sweep answers what no reasoning here can**. **Four failure statuses, because each sends somebody somewhere else:** ambiguous / unconfirmed / no_candidate / unidentifiable. `unconfirmed` is the interesting one — a single last-4 match is NOT linked, because that is precisely the guess the old code made. **0195 uses a new `fuel_card_link` column, not `sync_error`** — which resolves 7.5's linking half; only its refresh half remains. **The reading half matters as much:** `••••7552` names a GROUP here, and the overrides panel printed `driverName ?? unitPrompt`, hiding whichever it did not pick — often the only discriminator. 37 live cards have neither and now say so out loud. **Still to do: `supabase db push` and a live sweep.** The counts above are simulated from the mirror. The number to watch is how many rows come back `method: "pan_exact"`; zero would mean the PAN tiers are inert. |
| 2026-08-15 (night, proven) | 7 | **Step 7.7 proven live** · 0195 pushed · manual sweep | **54 → 157 of 197 live production cards linked in one sweep**, and `pan_exact` fired **103** times — so EFS and the fuel import DO store the same 19-digit number, which is the one thing the step could not check offline. The previous sweep, on the old last-4 linker, linked **2**. Fuel attribution went from 27% of the fleet to 80%. `sync_error='ambiguous_fuel_card_link'` went **139 → 0**, closing the half of Step 7.5 that was mislabelling successful refreshes. All 35 remaining ambiguous rows name the candidates they could not tell apart, which is the step's Verify. **The prediction was right about the number and wrong about the mechanism:** the pre-build simulation said the UNIT tier would resolve ~100 of 143; it resolved 0 and the PAN tier resolved 103. A matching headline would have hidden a completely different story — which is precisely why `method` is recorded per row instead of inferred from a total. **Keep the lower tiers anyway:** `last4_unit` fired zero times because the strongest tier reached everything it could, not because it is redundant. **The remaining 40 are a DATA ceiling, not a code one** — 29 have no `unit_prompt` and 5 have no `fuel_cards` counterpart, and each row now names its own blocker. **Operational note:** there is deliberately no refresh button (audit B6), `JOB_EXECUTION_MODE` is `inprocess` so there is no queue to drop a job into, and the scheduler runs every 24h — so `efs.mjs sync` / `efs.mjs job` were added, which is how this was triggered at all. |
| 2026-08-15 (night, last) | 4 | **Phase 4's suspension gate MEASURED** · `suspend-drill` added · `docs/22` H10 | **481 ms, the very next call** — `card_lock` suspended on QA, a real lock attempted immediately in the same process, refused `403 card_control_suspended`, then re-enabled. Verified from the DATABASE: QA back to `enabled`, card still ACTIVE, and **no mutation row created at all** — the gate refuses before the ledger opens. The figure is an upper bound dominated by one HTTPS round trip, which is what `promotionBlock`'s "deliberately uncached" comment predicts; it is now a measurement rather than a claim. **The first run suspended the WRONG COMPANY, and that is the finding.** The drill's safety was designed entirely around the CARD — a stale `expectedVersion`, so a failed suspension is still refused by optimistic concurrency and no write can land. That reasoning was correct and it held. It was also the wrong noun: told to use a QA token and handed a production one, nothing checked, so it suspended `card_lock` on the org where card control has never been promoted and then could not restore it, because a re-enable must cite a proof and production has none. Production kept one `card_lock = suspended` row **deliberately** — it refuses HARDER than the `not_promoted` it replaced, zero settings rows and zero mutation rows confirm card control was never on and no card was touched, and Phase 8 will clear it through the app. A SECOND defect meant that run measured nothing: no `Idempotency-Key`, which the routes validate as a uuid before the capability gate. **Both defects were in the drill; the product behaved correctly throughout** — the gate refused, the idempotency check refused, the promotion gate correctly refused to re-enable an unproven capability, and every step is in the audit log. **The lesson generalises: when a dangerous action is made safe, the thing made safe is whatever the author was picturing. Ask what else the action selects, not only what it does.** |
| 2026-08-15 (night, close) | 4 | **PHASE 4 CLOSED** · production echo scan re-run · `echo-scan` pages itself | **197 scanned, 197 passed, 0 failed, in ONE uninterrupted invocation** — six batches tiling 0–196 with no gap and no overlap, so the gate needs no arithmetic across runs and no assumption that the vendor's card order is stable between calls. An earlier two-run framing of the same result quietly depended on that assumption; the single clean run makes it moot. **Three things the audit log showed that the terminal could not.** Batch size is bound by the route's 60-second WALL CLOCK, not by `--limit` — every batch returned 28–34 against a requested 50, so the flag is inert above ~34 on this account. A client can lose a batch the SERVER completed: the crashed run's final batch has an audit row, so the retry added afterwards would have re-run vendor work rather than recovered lost work. And three runs cost **427 card reads to produce 197 cards of evidence**, which is the real price of a sweep that cannot resume — hence the resume path. **Phase 4's exit gate is now zero unchecked items**, and every one of them was OBSERVED rather than argued: proven and promoted on QA, the production refusal watched (and found a gate that named one blocker of three), suspension measured at 481 ms, echo scan green across the account. **Both live drills found defects the code review had not, and both were in the harness rather than the product.** |
...
**2026-09-23 — a proof left ••••6122 changed; the revert now retries, and a restore exists** (branches
`claude/efs-proof-revert-hardening`, merged, and `claude/efs-restore-from-ledger`). Proof `efe2b98a`
(`prompts_set`) flipped UNIT from EXACT_MATCH "669" to REPORT_ONLY. The revert's first read timed out at
10 s, and the card stayed changed, breaking rule 14. The cause took two defects together:
- `soapFetch` built its timeout signal once, so all four retries went out on an already-fired signal and
  failed in about 1 ms.
- The harness made one revert attempt and stopped.

Fixed:
- Each SOAP attempt now gets its own deadline, and a timeout is named as one (`SoapDeadlineError`).
- `harness/proofRevert.ts` retries the revert up to 3 times. The version fence stands in for the
  in-flight guard, which a proof's own rows are exempt from.
- The prompts flip carries its value into `reportValue`.
- New: `pnpm efs:restore --mutation-id <uuid>` (`harness/restore.ts`) replays a proof's `before_document`
  through the capability's own undo. It works only on a proof's apply row, only while the card still
  reads at `after_version`, and under the proof flags.
- Remediation of ••••6122 itself is still owed.
