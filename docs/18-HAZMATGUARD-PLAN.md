...
> v7 (2026-07-30, PM decision per Miki "proceed as suggested"): **D5 second source refined from
> hand transcription to automated official-edition triangulation.** `import/captureGovInfo.ts`
> captures the official GovInfo annual CFR edition (Title-49 GPO legal edition) as XML; `import/diff.ts`
> diffs it row-by-row against the eCFR parse, and differences are reconciled to Federal Register
> amendments / eCFR corrections. This does NOT drop v5's OFR-independence argument - the human check
> is *preserved* but shrinks from transcribing ~200 rows to attesting a machine-generated reconciliation
> report (+ spot-checking the official PDF). Strictly broader mechanical coverage (all rows) with a
> smaller, less error-prone human step. Threaded: D5 row; H1 deliverable 3 (diff second source =
> GovInfo edition); RELEASING.md capture->diff->reconcile->attest step. `fixtures/handVerifiedRows.ts`
> retained as an optional spot-check anchor, no longer the second source of record.
...
- **The clearing RULES are enforced SERVER-SIDE** (`@silvicom/shared/hazmatReview.ts` — `checkHazmatClear`,
  ...
  now loads the run's flags + the load's special permits and refuses via `checkHazmatClear` — a violation
  demands an override reason ≥ 20 chars (and writes an explicit `override` review row for the H12 report), a
  special-permit load demands the SP acknowledgement, and an **unusable-read run** (`recapture_needed` /
  `extraction_failed` / `no_documents` / `document_unreadable` / aborted paths) or a **provisional dataset**
  can't clear AT ALL — the reviewer must reject/re-run, never override a read that never happened. The
...
3. **Second-source diff** (`import/diff.ts`) *(revised per D5 v5/v7 - official-only; second source now automated GovInfo triangulation, human reduced to attestor)*: loads the
   ...
   *Automated official-edition triangulation (D5 v7 - the primary mechanical gate; BUILT + CLEAN 2026-07-30, `import/govinfoCrossCheck.ts`):* `import/captureGovInfo.ts` captures the GovInfo annual CFR edition XML and `govinfoCrossCheck.ts` cross-parses it against the eCFR XML row-by-row - exhaustive over ALL rows (not just the transcribed subset), catching every parse/transport error and flagging amendment drift (reconciled to the Federal Register + eCFR corrections). It still does NOT satisfy the *source*-data independence requirement on its own (same OFR origin), so the human check above is preserved but reduced to **attesting the reconciled report** (+ spot-checking the PDF) rather than transcribing every row. `handVerifiedRows.ts` remains an optional spot-check anchor.
