...
import {
  runAllRules, correlateSignals, CASE_RULE_ID,
  type RuleContext, type RuleResult, type RuleId,
...
import { loadVehicleContext, loadConsumptionContext, loadReeferContext, loadReeferDiversionContext } from "./context.js";
...
export async function scoreTransaction(
  ...
  const { vehicle, samsaraVehicleId, odometerOffsetSource } = await loadVehicleContext(admin, txn.vehicleId);
  ...
  const recon = await resolveReconciliation(admin, env, orgId, r, txn, vehicle, samsaraVehicleId, opts);
  ...
  const { reeferTankCapacityGal, reeferWindowGallons } = await loadReeferContext(admin, orgId, txn, r, winStartIso);
  const { reeferPaired, orgUsesReeferFuel, reeferDiversionReeferGal, reeferDiversionTractorGal, reeferLoadInWindow } =
  ...
  await persistTxnOutcome(admin, txnId, { txn, previousTxn, intermediateGallons, assessment, ruleCtx, recon });
  await learnAndUpdateVehicle(admin, txn, vehicle, samsaraVehicleId, odometerOffsetSource, recentTxns, opts);
}
