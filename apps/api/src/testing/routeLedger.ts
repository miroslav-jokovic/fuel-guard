...
export const OPEN_ROUTES = new Map<string, string>([
  ...
  [
    "GET /api/fueling/findings/summary",
    "the open-findings and recovered-this-quarter figures on the Dashboard's fuel strip (C9). Gated per ROW like the inbox it summarises: `visibleSections` derives the caller's sections from the matrix, the anomaly table is not counted without `safety`, and the money figure — which only the ledger can produce, since D-FUI7 gives an anomaly none — is null without `fuel`. A caller who may see neither is answered `null` rather than 403, because the Dashboard has no section gate of its own and refusing one strip on a page that refuses nothing else would be a gate introduced to suit a tile (the Q-SAM7 ruling, applied again here)",
  ],
  [
