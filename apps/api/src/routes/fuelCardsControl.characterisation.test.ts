...
describe("the bytes each write route puts on the wire", () => {
  ...
  it("locks a card to Hold", async () => {
    expect(await dispatchOf(F, `${R}/lock`, "POST", (v) => ({ expectedVersion: v, status: "Hold" })))
  ...
  it("unlocks a card back to Active", async () => {
    expect(await dispatchOf(F, `${R}/unlock`, "POST", (v) => ({ expectedVersion: v })))
  ...
  it("grants a single-location override for three uses", async () => {
    expect(await dispatchOf(F, `${R}/override`, "POST", (v) => ({ expectedVersion: v, uses: 3, scope: { kind: "location", locationId: "115732" } })))
  ...
  it("clears an override that was actually set", async () => {
    expect(await dispatchOf(O, `${R}/override`, "DELETE", (v) => ({ expectedVersion: v })))
  ...
  it("changes one prompt and echoes the rest", async () => {
    expect(await dispatchOf(F, `${R}/prompts`, "POST", (v) => ({ expectedVersion: v, replaceAll: true, prompts: [{ infoId: "DRID", validationType: "EXACT_MATCH", matchValue: "D-9999", reportValue: null, remove: false }] })))
