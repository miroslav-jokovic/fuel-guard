...
import { aggregateDriverIdle, type IdleRow, type IdleSummary, type IdleClassification } from "@fuelguard/shared";
    ...
    queryFn: async (): Promise<IdleSummary> => {
      ...
      for (let offset = 0; ; offset += PAGE) {
        const { data, error } = await supabase
          .from("idle_events")
          .select("driver_id, duration_sec, classification, fuel_gal, cost_usd, drivers(full_name)")
          .gte("started_at", from)
