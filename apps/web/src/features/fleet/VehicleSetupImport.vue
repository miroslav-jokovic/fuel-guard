...
<template>
  <div class="space-y-5">
    ...
    <template v-if="!preview">
      ...
      <!-- How-to steps -->
      <ol class="space-y-1 rounded-surface bg-brand-50 px-4 py-3 text-sm text-brand-800 ring-1 ring-brand-100 list-decimal list-inside">
      ...
      <div class="grid grid-cols-2 gap-3">
        <BaseButton
          class="flex flex-col items-center gap-2 rounded-surface border-2 border-dashed border-brand-300 px-4 py-5 text-center transition-colors hover:border-brand-400 hover:bg-brand-50"
        ...
        >
          <AppIcon :icon="CsvIcon" class="size-6 text-ink-tertiary" aria-hidden="true" />
      ...
      <!-- Field reference -->
      <details class="rounded-surface bg-surface-subtle ring-1 ring-edge">
        ...
        <div class="overflow-x-auto border-t border-edge px-4 pb-3 pt-2">
          <AppTable class="min-w-full text-xs">
            <thead>
            ...
            <tbody class="divide-y divide-edge-subtle text-ink-secondary">
              ...
              <tr><td class="py-1.5 pr-4 font-mono">make</td><td class="pr-4 text-ink-tertiary">No</td><td>e.g. Freightliner, Ford, Kenworth</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">model</td><td class="pr-4 text-ink-tertiary">No</td><td>e.g. Cascadia, F-450, T680</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">year</td><td class="pr-4 text-ink-tertiary">No</td><td>4-digit model year (e.g. 2021)</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">plate</td><td class="pr-4 text-ink-tertiary">No</td><td>License plate number</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">vin</td><td class="pr-4 text-ink-tertiary">No</td><td>17-character VIN</td></tr>
              ...
              <tr><td class="py-1.5 pr-4 font-mono">current_odometer</td><td class="pr-4 text-ink-tertiary">No</td><td>Current odometer reading in miles</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">status</td><td class="pr-4 text-ink-tertiary">No</td><td>active / maintenance / retired (default: active)</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">apu_type</td><td class="pr-4 text-ink-tertiary">No</td><td>diesel_apu / battery_hvac / fuel_heater / shore_power / none — idle-reduction equipment</td></tr>
              <tr><td class="py-1.5 pr-4 font-mono">has_optimized_idle</td><td class="pr-4 text-ink-tertiary">No</td><td>yes / no — OEM optimized idle (e.g. Freightliner Cascadia)</td></tr>
            </tbody>
          </AppTable>
      ...
      <FileDropzone
        ...
        hint="Upload the completed template to preview and validate before importing"
        @files="onFile"
      />
    ...
    <template v-else>
      ...
      <p class="truncate text-xs text-ink-muted">
        ...
        <span class="ml-2 text-ink-tertiary">{{ preview.totalRows }} data row{{ preview.totalRows === 1 ? "" : "s" }}</span>
      </p>

      <!-- Summary stats -->
      <dl class="grid grid-cols-3 gap-3">
        <div class="rounded-surface p-3 ring-1" :class="preview.toCreate.length ? 'bg-success-50 ring-success-200' : 'bg-surface-subtle ring-edge'">
          <dt class="text-xs" :class="preview.toCreate.length ? 'text-success-700' : 'text-ink-muted'">New vehicles</dt>
          <dd class="text-2xl font-bold" :class="preview.toCreate.length ? 'text-success-800' : 'text-ink-tertiary'">{{ preview.toCreate.length }}</dd>
        </div>
        <div class="rounded-surface p-3 ring-1" :class="preview.toUpdate.length ? 'bg-brand-50 ring-brand-200' : 'bg-surface-subtle ring-edge'">
          <dt class="text-xs" :class="preview.toUpdate.length ? 'text-brand-700' : 'text-ink-muted'">Setup updates</dt>
          <dd class="text-2xl font-bold" :class="preview.toUpdate.length ? 'text-brand-800' : 'text-ink-tertiary'">{{ preview.toUpdate.length }}</dd>
        </div>
        <div class="rounded-surface p-3 ring-1" :class="preview.errors.length ? 'bg-danger-50 ring-danger-200' : 'bg-surface-subtle ring-edge'">
          <dt class="text-xs" :class="preview.errors.length ? 'text-danger-700' : 'text-ink-muted'">Errors</dt>
          <dd class="text-2xl font-bold" :class="preview.errors.length ? 'text-danger-800' : 'text-ink-tertiary'">{{ preview.errors.length }}</dd>
        </div>
      </dl>

      <!-- Errors -->
      <div v-if="preview.errors.length" class="max-h-28 overflow-y-auto rounded-surface bg-danger-50 px-3 py-2 text-xs text-danger-700 ring-1 ring-danger-200">
        <p v-for="(err, i) in preview.errors" :key="i">{{ err }}</p>
      </div>

      <!-- New vehicles table -->
      <div v-if="preview.toCreate.length" class="overflow-hidden rounded-surface ring-1 ring-edge">
        ...
        <div class="overflow-x-auto">
          <AppTable class="min-w-full divide-y divide-edge-subtle text-sm">
          ...
          <p v-if="preview.toCreate.length > 50" class="px-3 py-2 text-xs text-ink-tertiary">+{{ preview.toCreate.length - 50 }} more…</p>
        </div>
      </div>

      <!-- Setup updates table -->
      <div v-if="preview.toUpdate.length" class="overflow-hidden rounded-surface ring-1 ring-edge">
        ...
        <div class="overflow-x-auto">
            ...
            <tbody class="divide-y divide-edge-subtle">
              <tr v-for="c in preview.toUpdate.slice(0, 50)" :key="c.id" class="hover:bg-surface-subtle">
                ...
                <td class="px-3 py-1.5 text-ink-secondary">
                  <span :class="c.tank_before !== c.tank_after ? 'text-ink-tertiary line-through' : ''">{{ fmt(c.tank_before) }}</span>
                  <span v-if="c.tank_before !== c.tank_after" class="font-medium text-ink"> → {{ fmt(c.tank_after) }}</span>
                </td>
                <td class="px-3 py-1.5 text-ink-secondary">
                  <span :class="c.mpg_before !== c.mpg_after ? 'text-ink-tertiary line-through' : ''">{{ fmt(c.mpg_before) }}</span>
          ...
          <p v-if="preview.toUpdate.length > 50" class="px-3 py-2 text-xs text-ink-tertiary">+{{ preview.toUpdate.length - 50 }} more…</p>
        </div>
      </div>
      ...
      <div
        ...
        class="rounded-surface bg-surface-subtle px-4 py-4 text-center text-sm text-ink-muted ring-1 ring-edge"
      >
