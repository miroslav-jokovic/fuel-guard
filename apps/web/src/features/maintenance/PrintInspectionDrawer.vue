 ...
 * millimetres low. It replaces the body rather than stacking a second overlay, which is contract
 * §6.2's rule for the same reason a drawer is not a modal — one decision on screen at a time.
 */

const props = defineProps<{ open: boolean; inspectionId: string; canManage: boolean }>();
...
/**
 * Opened as a blob rather than navigated to: these routes sit behind `requireAuth`, and a plain
...
const printAlignmentSheet = () =>
  openPdf(`/api/maintenance/printing/registration-sheet.pdf${profileId.value ? `?profile=${profileId.value}` : ""}`);
...
async function saveSetup() {
  try {
    ...
    toast.success("Printer setup saved");
    setupOpen.value = false;
  } catch (e) {
    toast.error("Could not save the printer setup", e instanceof Error ? e.message : undefined);
  }
}
</script>

<template>
  <SlideOver :open="open" size="lg" title="Print inspection" @close="emit('close')">
    <div class="space-y-6">
      <section v-if="!setupOpen" aria-labelledby="print-target">
        ...
        <div class="mt-4 space-y-4">
          ...
          >
            <ComboSelect :id="id" v-model="profileId" :options="profileOptions" placeholder="Choose a printer" />
          </FormField>
        </div>
    ...
    <template #footer>
      <div class="flex items-center justify-end gap-3">
        <template v-if="setupOpen">
          <BaseButton variant="ghost" @click="setupOpen = false">Back</BaseButton>
          <BaseButton
            variant="primary"
            :disabled="!form.name.trim() || saveProfile.isPending.value"
            @click="saveSetup"
          >
            {{ saveProfile.isPending.value ? "Saving…" : "Save printer" }}
          </BaseButton>
        </template>
        <template v-else>
