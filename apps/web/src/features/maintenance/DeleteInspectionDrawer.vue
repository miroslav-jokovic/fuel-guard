 ...
 * filed PDF and the object in storage all go, and the truck's inspection date goes back to what the
...
async function confirmDelete() {
  try {
    ...
    toast.success(
      "Record deleted",
      result.expiresOn
        ? `The truck's inspection now expires ${result.expiresOn}, from the report that remains.`
        : "The truck no longer carries an annual inspection date.",
    );
    emit("deleted");
  } catch (e) {
    ...
    <div class="space-y-6">
      <AppCallout tone="danger">
        ...
        <p class="mt-1">
          ...
          removed, and the file is deleted from storage. The truck's inspection date goes back to
          whatever its remaining reports justify.
        </p>
