...
export function buildAttentionStrip(rows: readonly DriverOverviewRow[]): AttentionTile[] {
  ...
  return [
    { key: "expired", label: "Expired", n: rows.filter((d) => d.counts.expired > 0).length, tone: "danger", state: "expired", due: "" },
    { key: "due14", label: "Due in 14 days", n: dueWithin(14), tone: "warning", state: "", due: "14" },
    { key: "due30", label: "Due in 30 days", n: dueWithin(30), tone: "warning", state: "", due: "30" },
    { key: "not_started", label: "Not started", n: rows.filter((d) => d.state === "not_started").length, tone: "neutral", state: "not_started", due: "" },
    { key: "complete", label: "Files complete", n: rows.filter((d) => d.state === "complete").length, tone: "success", state: "complete", due: "" },
  ];
}
