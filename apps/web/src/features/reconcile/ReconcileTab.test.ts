...
const SYSTEM: SystemFill[] = [
  { id: "s1", cardRef: "7083050030491234", controlId: null, unit: "701", fueledAt: "2026-08-17T14:00:00Z", tranDate: "2026-08-17", gallons: 120, totalCost: 500 },
  // Same card and day as r2 below, billed $40 less than the vendor says — an amount mismatch.
  { id: "s2", cardRef: "7083050030495678", controlId: null, unit: "754", fueledAt: "2026-08-18T14:00:00Z", tranDate: "2026-08-18", gallons: 90, totalCost: 430 },
  // Recorded by us and absent from the report entirely.
  { id: "s3", cardRef: "7083050030499999", controlId: null, unit: "812", fueledAt: "2026-08-19T14:00:00Z", tranDate: "2026-08-19", gallons: 80, totalCost: 350 },
];
...
describe("ReconcileTab", () => {
  it("asks for a file before it claims anything", () => {
    ...
    expect(t).not.toContain("Discrepancies");
  ...
  it("reconciles a dropped report against the recorded fills and names each bucket", async () => {
    ...
    expect(t).toContain("Discrepancies");
    expect(t).toContain("Missing in our system"); // a3: on the report, never recorded
    expect(t).toContain("Missing on report");     // s3: recorded, never billed
    expect(t).toContain("Amount mismatch");       // a2 vs s2, $40 apart
    expect(t).toContain("Matched clean");         // a1 vs s1
    expect(t).not.toContain("NaN");
  });
  ...
  it("renders a status as a word, never as its token", async () => {
    ...
    const t = (await withReport()).text();
    expect(t).toContain("Missing (system)");
  ...
  it("opens on the discrepancies rather than on a table the reader has to filter", async () => {
    ...
    expect(w.text()).toContain("Discrepancies only");
  ...
  it("shows every row, clean ones included, once the reader asks for them", async () => {
    ...
    const cleanTile = w.findAll("button").find((b) => b.text().includes("Matched clean"));
