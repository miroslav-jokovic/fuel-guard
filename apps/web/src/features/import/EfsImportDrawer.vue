...
/**
 * Backfill a period from an EFS report file (FUEL-C4, D-FUI3).
 *
 * ── WHY THIS IS A DRAWER AND NO LONGER A PAGE ───────────────────────────────────────────────────
 * This was `/import`, a top-level nav item titled with a verb applied to a file format. C1 already
 * measured why that was wrong as a PAGE: ALL 28,484 `efs_transactions` rows in production carry
 * `imports.source = 'efs_feed'` (2026-09-01) and not one has ever arrived through this form. The
 * poller acquires the rejected feed every 5 minutes and the posted feed every 15. A nav item is a
 * claim about how often something is used, and this one was making the wrong one — teaching a manual
 * habit for an automated pipeline, then reassuring the reader afterwards that the duplicate was
 * harmless.
 *
 * So the capability is unchanged and its placement is not: it opens from the Fuel Log's header,
 * beside the log it backfills, and it is named for the gap it fills rather than for the file it eats.
 *
 * ⚠ **The button that opens it is gated on `can("fuel")`, and that is not decoration.** `/import`
 * was catalogued `manage("fuel")` while `/fuel-log` is `always`, so moving this onto that page
 * without the check would offer a write to `recruiter` and `technician` — both `fuel: "none"` — and
 * both already reach `/fuel-log`. The API refuses them (`requireSection("fuel")` on
 * `/api/transactions/import-report`), so the widening would have been an action that 403s rather
 * than a data leak; an action offered and then refused is still a defect. Same shape as the two tabs
 * C2 absorbed, and the gate is read from the same matrix.
 *
 * The body below is the old page's EFS tab, moved rather than rewritten. What did NOT come with it is
 * **Repair fuel data**, which moved to Settings → Data & Sync where the other repair actions already
 * live and where it gains the progress and freshness a job-backed action should have had.
 */
...
import { analyzeImport, useCommitImport, type ImportPreview } from "./useImport";
...
<template>
  <SlideOver
    :open="open"
    size="lg"
    title="Backfill EFS reports"
    description="For a period the feed never carried, or one that predates it."
    @close="emit('close')"
  >
    <div class="space-y-6">
      <!--
        C1, FUEL-SECTION-CONSOLIDATION-PLAN §0.2 fact 7. This used to open with "Upload your EFS
        ...
        one has ever arrived through this form.

        Telling somebody to upload a file the worker already fetched is not a small copy problem: it
        is the surface teaching a manual habit for an automated pipeline, and then the
        duplicate-detection note below reassuring them it was harmless. So it says what it is FOR — a
        gap, or a period predating the feed — and leaves the routine path alone.
      -->
      <p class="text-sm text-ink-muted">
        ...
        scored as it lands. Use this for a <strong>gap</strong>: a period the feed never carried, or
        ...
        <BaseCard v-for="(preview, i) in previews" :key="preview.filename" class="space-y-5">
          ...
          <div v-if="preview.alreadyImported" class="flex items-start gap-3 rounded-control bg-warning-50 px-4 py-3 text-sm text-warning-800 ring-1 ring-warning-200">
            ...
            <p>
              ...
              but the dashboard is missing days, use <em>Repair fuel data</em> in Settings → Data &amp; sync instead.
            </p>
          </div>
    ...
    <!--
      The queue's actions, in the drawer's footer rather than in a card at the end of the list
      (apps/web/CLAUDE.md: "SlideOver (actions in #footer)"). It matters more here than on a page:
      the review list is as long as the number of files dropped, and a Commit button that scrolls
      away below three previews is a Commit button somebody cannot find.
    -->
    <template v-if="previews.length" #footer>
