...
import { enqueue, dequeue, flush, pending } from "@/features/inventory/countQueue";
...
/**
 * Send what is on this phone, oldest first. Called on reconnect and after each entry — the second
 * is what makes the queue usually empty, so the strip is honest when it says nothing is waiting.
 */
async function drain() {
  await flush((movement) => record.mutateAsync(movement).then(() => undefined));
  await refreshQueue();
}
...
async function commit() {
  ...
  // Written to the phone BEFORE the network is touched. This ordering is the feature.
  await enqueue({
    id: movement.data.id,
    sessionId: sessionId.value,
    movement: movement.data as PartMovementInput,
  ...
  try {
    await record.mutateAsync(movement.data as PartMovementInput);
    await dequeue(movement.data.id);
  } catch {
    // Kept on the phone; the strip says so and `drain()` will replay it in order.
  }
  await refreshQueue();
...
async function undo(key: string) {
  ...
  const entry = rows.find((r) => `${r.movement.partId}:${r.movement.locationId}` === key);
...
<template>
  <div class="space-y-4">
    ...
    <template v-else-if="walk">
      ...
      <header class="sticky top-0 z-raised -mx-4 border-b border-edge bg-surface px-4 py-3">
        <p class="text-lg font-semibold text-ink">{{ walk.holderLabel ?? "Count" }}</p>
        <p class="mt-0.5 text-sm text-ink-secondary">
          {{ doneCount }} of {{ lines.length }}<span v-if="shortCount"> · {{ shortCount }} short</span>
          <span v-if="blindNow" class="text-ink-tertiary"> · blind</span>
        </p>
      </header>

      <!-- The promise the queue keeps. Shown only when something is actually waiting. -->
      <div v-if="queued > 0" class="rounded-control bg-warning-50 px-3 py-2 text-xs text-warning-700 ring-1 ring-inset ring-warning-600/20">
