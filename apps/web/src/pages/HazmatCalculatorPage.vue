 ...
 * and the deterministic engine (D1) returns placards, ID displays, segregation and eligibility with CFR
 * citations, no AI in the path. Stateless: nothing is saved (the Load Workspace persists, later). Free-text
 * products are impossible — a line resolves against the HMT before it can be added (fail-closed at source).
 */
const form = reactive<CalcForm>(emptyForm());
const calc = useHazmatCalc();
...
async function calculate() {
  const res = await calc.mutateAsync(buildCalcRequest(form));
  result.value = res;
}
  ...
  <div class="space-y-6">
    <PageHeader description="Enter a load by hand to get required placards, ID displays, segregation and citations.">
      <template #actions>
        ...
        <BaseButton variant="soft" size="sm" @click="resetAll">Reset</BaseButton>
      </template>
    </PageHeader>
      ...
      <form class="space-y-4" @submit.prevent="calculate">
        <BaseCard>
          ...
          <div class="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <FormField v-slot="{ id }" label="Previous/current business-day IDs" hint="§172.336(c): IDs retained from the last load, e.g. UN1203. Comma-separated.">
