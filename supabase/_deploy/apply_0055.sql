-- ────────────────────────────────────────────────────────────────────
-- MANUAL DEPLOY — 0055 driver_performance_weeks (docs/16-DRIVER-PERFORMANCE.md)
-- Copy-paste into the Supabase SQL editor and run. Idempotent.
-- Frozen, settled weekly leaderboard + winners (the rewards ledger). Member-read; mgr write.
-- ────────────────────────────────────────────────────────────────────
...
-- Verify:
-- select org_id, week_start, count(*) filter (where is_winner) as winners from driver_performance_weeks group by 1,2;
